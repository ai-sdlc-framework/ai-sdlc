...
#
# AISDLC-490 B+: we add a third idempotency check — whether any .v6.dsse.json
# (or .dsse.json for v5) file is already committed into HEAD's git tree. This
# handles the case where patch-id is empty (e.g. diff is attestation-files-only
# after exclusions) and the SHA changed after a prior amend: the envelope is
# in the tree even though it's named after the pre-amend SHA. Without this
# check, the hook would re-sign on every subsequent push after an amend-only
# scenario (no code diff).
if [ -f "$ATT_FILE" ] || { [ -n "$ATT_FILE_LEGACY" ] && [ -f "$ATT_FILE_LEGACY" ]; }; then
  ...
  # push aborted and operator re-pushed, or the operator pre-signed manually.
  exit 0
fi
...
# ── Step 6: stage + amend the envelope into the current commit ────────
# AISDLC-490 B+ end-state: instead of creating a separate chore commit that
# moves HEAD (which caused the re-sign loop and required a second `git push`),
# we AMEND the current commit to include the attestation files. This keeps
# HEAD at the same logical position (same commit count) and makes the loop
# structurally impossible: there is never a "signed dev commit without envelope
# on top" state because the envelope IS the dev commit.
#
# Why `--no-edit`: the current commit message is the developer's conventional-
# commit message. We preserve it exactly — the attestation files are metadata
# and do not belong in the commit subject/body per AISDLC-220 semantics.
#
# `--no-verify` skips pre-commit (lint-staged has nothing to do with a
# generated JSON envelope) and does NOT run pre-push recursively (amend does
# not invoke pre-push hooks — that only fires on `git push`).
#
...
# The `[ -d ]` guard around the `git add` below is MANDATORY, not merely
# defensive: this script runs under `set -euo pipefail`, and `git add` on a
# non-existent path exits 128 (fatal: pathspec did not match), which would
...
# skip the stage and amend only the envelope.
(
  cd "$WT_ROOT"
  ...
  # they skip this stage and still just amend the envelope.
  if [ -d "$WT_ROOT/.ai-sdlc/transcript-leaves" ]; then
    git add -- "$WT_ROOT/.ai-sdlc/transcript-leaves/"
  fi
  # AISDLC-490 B+: amend the current commit to include the attestation files.
  # This is the key change from the old chore-commit approach: HEAD SHA changes
  # (amend creates a new commit object) but no NEW commit is created — the push
  # range stays at one commit and the re-sign loop is structurally impossible.
  git commit --amend --no-edit --no-verify >&2
) || {
  echo "[attestation-sign] ERROR: git add/amend of attestation failed; aborting push" >&2
  exit 2
}

# ── Step 7: exit 0 — push proceeds with the amended commit ────────────
# AISDLC-490 B+: because we amended HEAD (not added a new commit), the push
# can proceed immediately. The amended commit contains the attestation envelope
# + transcript-leaves alongside the source changes. No second `git push` is
# needed. The next push after any HEAD movement is idempotent: the idempotent
# check at Step 4 finds the patch-id envelope and exits 0.
#
# AI_SDLC_INTERNAL_NO_EXIT_1 is preserved for backward-compat (e.g. callers
# in test mode) but has no functional difference here since we exit 0 in both
# paths.
echo "[attestation-sign] Attestation staged into amended commit at $HEAD_SHA — push proceeds." >&2
echo "[attestation-sign] (B+ end-state, AISDLC-490: no chore commit, no re-push required)" >&2
echo "[attestation-sign] Defer with: AI_SDLC_SKIP_ATTESTATION_SIGN=1 git push" >&2
