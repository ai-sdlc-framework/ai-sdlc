...
| v4.1 | 2026-05-04 | Adds §14.2.1 making the HC_design principal-participation four-state semantic normative (AISDLC-185, doc backport of AISDLC-171). Documentation-only. |
...
> **v4.1 clarification (2026-05-04, AISDLC-185).** RFC-0009 §13 OQ-8 surfaced an adopter-visible ambiguity: when an issue's `pillarBreakdown.shared.hcComposite.design` value is `0`, the operator could not tell whether the project had no Design Authority configured at all, or whether one was configured but did not participate in this particular issue. Both states produce `hcDesign = 0`, but they have different operational meanings. AISDLC-171 resolved this by surfacing a `designAuthorityConfigured` diagnostic flag rather than altering the HC_design weight; this sub-section makes the resulting states normative.

`HC_design` exposes **four observable states** at `pillarBreakdown.shared.hcComposite`, distinguished by the `designAuthorityConfigured` diagnostic flag (which is `undefined`, `false`, or `true`) combined with `hcDesign`. The flag does NOT participate in the weight calculation; it is purely a reporting surface.

The "no DA configured" semantic groups two distinct underlying conditions — DSB-absent and DSB-with-empty-principals — into a single operator response category. The flag value differs (the implementation surfaces `undefined` vs `false`), but the operational meaning is identical: the project has not declared any Design Authority principals, and the design channel is intentionally inert. Tooling that classifies states should therefore handle both `undefined` (omitted) and `false` as "none".

| State | `designAuthorityConfigured` | `hcDesign` | When it appears | Meaning |
|---|---|---|---|---|
| **none** — DSB absent | `undefined` (field omitted) | `0` | No `DesignSystemBinding` is supplied to the enrichment context (`preDesignSystem` lifecycle phase per §6.2). `buildDesignAuthoritySignal` short-circuits and returns `undefined`; `computeAdmissionHumanCurve` then omits the flag entirely (see `orchestrator/src/admission-hc.ts` line 141). | No penalty. The project has not opted into Design Authority signal routing. Common for early-stage adopters. |
| **none** — DSB present, principals empty | `false` (field present and explicitly `false`) | `0` | The DSB exists but `stewardship.designAuthority.principals` is empty (or absent). `buildDesignAuthoritySignal` returns `{ isDesignAuthority: false, principalsDeclared: false }`; the flag propagates as `false` through `computeAdmissionHumanCurve` and surfaces on the breakdown. | No penalty. Operationally equivalent to DSB-absent — no Design Authority is declared for this project. The distinct flag value lets operators see that a DSB resource exists but has not yet been populated with principals. |
...
**Implementation anchors.** The four-state semantic is produced and surfaced in:

- [`orchestrator/src/admission-enrichment.ts`](../../orchestrator/src/admission-enrichment.ts) — `buildDesignAuthoritySignal` (lines 309-345) is the **origin** of the flag's three observable values: returns `undefined` when no DSB is supplied (DSB-absent state), returns `{ isDesignAuthority: false, principalsDeclared: false }` when the DSB has empty principals (DSB-present-empty-principals state), and returns `principalsDeclared: true` when the DSB declares one or more principals.
- [`orchestrator/src/admission-hc.ts`](../../orchestrator/src/admission-hc.ts) — `computeAdmissionHumanCurve` plumbs the flag from `DesignAuthoritySignal.principalsDeclared` onto `AdmissionHumanCurveResult.designAuthorityConfigured`. Per line 141, the field is only omitted when the source is `=== undefined`; the `false` value propagates through to the breakdown.
- [`orchestrator/src/pillar-breakdown.ts`](../../orchestrator/src/pillar-breakdown.ts) — `computePillarBreakdown` (lines 169-186) mirrors the same omit-only-on-undefined rule onto `pillarBreakdown.shared.hcComposite.designAuthorityConfigured`, so consumers can distinguish all four states at the breakdown surface.
