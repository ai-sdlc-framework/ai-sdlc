...
- **OQ-3 — Transport.** Pre-filled GitHub issue the human submits (RFC-0025 pattern, zero infra) vs an aggregated ping to an ai-sdlc-owned endpoint (better analytics, but reintroduces the telemetry surface RFC-0025 rejected) vs a single dedup'd issue-per-repo. Recommend the pre-filled-issue pattern for v1.
  ...
  **Resolution (2026-09-05, full rubric — OQ-4 folded in): a single dedup'd, label-quarantined, opt-in, anonymized GitHub issue PER ADOPTER REPO, updated in place rather than re-filed.** Default submission is a **tokenless pre-filled URL** the operator clicks (reusing RFC-0025's shipped anonymizer + consent preview); it upgrades to a `search-existing-and-update` GitHub-API path ONLY when the adopter explicitly grants an `issues:write` token, and degrades gracefully to "here is the existing issue link" otherwise. Industry research: no mainstream CLI auto-files issues from user machines (universally treated as spam); the two real patterns are opt-out analytics endpoints (Next.js/Gatsby/VS Code/Homebrew — recurring trust backlash) and manual pre-filled-issue-URL feedback (GitHub-native). **Counter-argument:** "for an analytics question a proper endpoint (Option B) is the honest tool; issue-as-database is an anti-pattern, and the update path needs a token to a repo the adopter doesn't own." Rebuttal: the tokenless-URL default sidesteps the token ask, and B's hosting + privacy/GDPR liability + reversal of RFC-0025's deliberate no-endpoint stance costs more than ≤1 labeled issue per adopter repo at current scale (28 forks); B is deferred behind a future OQ that auto-triggers on ≥N adopter requests (mirrors RFC-0043's escalation pattern). **Selected over Option B** (endpoint) to preserve the no-endpoint principle + zero infra/liability while still yielding aggregate signal via label search; **selected over Option A** (manual per-run issue) and **Option D** (Discussions) because those give the maintainer essentially no *aggregate* visibility — the actual goal — and A carries unbounded tracker-spam risk. This structurally resolves OQ-4: one issue per repo, updated in place, is the anti-DoS mechanism.
...
- **OQ-4 — Anti-DoS / dedup.** How to stop N adopters filing N identical issues: search-existing-issue-and-+1 vs aggregate-then-summarize vs a maintainer-side intake bot that dedups by fingerprint. This is the load-bearing operational decision — an un-deduped design DDoSes the project's own tracker.
  ...
  **Resolution (2026-09-05): folded into OQ-3.** The chosen transport — one dedup'd issue PER ADOPTER REPO, updated in place — IS the dedup strategy (bounded at one open issue per repo, never re-filed). Implementation detail for the tasks: dedup is keyed by a stable per-repo fingerprint (anonymized repo identifier + findingSet hash) so a re-run with the same findings updates the existing issue and a changed finding-set opens/updates exactly one. The maintainer-side intake-bot / aggregate-then-summarize variants remain available as a later enhancement if label-search aggregation proves insufficient (the same ≥N-adopter escalation trigger as OQ-3's Option B).
