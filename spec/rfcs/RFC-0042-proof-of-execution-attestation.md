...
**Honest limit of `verdictClass`.** This is a same-process/session HEURISTIC, not a cryptographic proof. On a single machine with a coordinator that already has full repo Bash/Write access, a sufficiently determined coordinator could fabricate a marker file directly — including forging a reviewer `agentType` string — bypassing the need to actually spawn a subagent. What this DOES defend against is the common, non-adversarial case: a coordinator running the reviewer's prescribed Bash steps itself (in the same conversation turn), or reusing an unrelated real marker from a non-reviewer subagent, rather than dispatching a real reviewer `Agent`/`Task` subagent — that case now correctly downgrades to `self-authored`. Closing the adversarial-coordinator gap (a coordinator willing to forge `agent_type` itself) requires binding the leaf to the reviewer subagent's own harness-captured execution transcript (DEC-0012 opt-a) — tracked as a harness-coupled follow-up, AISDLC-570, not implemented here.
