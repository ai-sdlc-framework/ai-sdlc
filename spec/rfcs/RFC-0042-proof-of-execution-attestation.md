...
**What this does NOT prove (honest scope, AISDLC-568).** Forgery resistance is not the same claim as reviewer *independence*. The transcript-capture contract (Step 0 / Step END in each reviewer agent body) is a set of Bash commands the reviewer subagent's own process runs against its own conversation — nothing structurally distinguishes a transcript written by a genuinely separate reviewer subagent from one written by the *same coordinator process* that also drives the developer subagent and makes the ship decision, provided that coordinator is willing to pay the same LLM-token cost to produce the transcript content. PoE raises the cost of an *unexecuted* review to the cost of a real one; it does not, by itself, structurally anchor that the process producing the transcript is independent of the process deciding to merge. See the whitepaper's [§2.2 Out of scope](../../docs/whitepapers/proof-of-execution.md#22-out-of-scope-poe-does-not-defend-against-these) table for the equivalent caveat.

**`verdictClass` (AISDLC-568, DEC-0012 opt-b, shipped; role-bound since AISDLC-572).** Every v6 transcript leaf now carries a `verdictClass: 'independent' | 'self-authored'` field. The signal: Claude Code's `SubagentStart` hook (`ai-sdlc-plugin/hooks/subagent-start.js`) fires ONLY when the harness itself dispatches a subagent via the `Agent`/`Task` tool — it cannot be triggered by a coordinator merely writing files or emitting text. On each firing, the hook writes a marker file to `.ai-sdlc/subagent-sessions/<agent-id>.json` recording `agentId`, `agentType` (from the `SubagentStart` payload's `agent_type` field), and `firedAt`. `cli-attestation emit-leaf` checks for an unconsumed marker whose `agentType` is a REVIEWER role (`code-reviewer`, `test-reviewer`, `security-reviewer`, `code-reviewer-codex`, `test-reviewer-codex` — explicitly NOT `developer`, `rebase-resolver`, `ci-conflict-resolver`, or `refinement-reviewer`) AND whose firing time is within a bounded window (30 minutes) of the transcript's last-write time; a match classifies the leaf `independent` (and consumes the marker so it cannot back-stop a second leaf); no match (wrong role, missing/legacy `agentType`, stale, or any error) classifies it `self-authored` — the fail-safe default. The verifier (`verifyV6Envelope`) cross-checks the envelope's declared `verdictClass` against the on-disk leaf's Merkle-proved value (rejecting a mismatch as tampering) and surfaces a per-leaf breakdown plus an `overallVerdictClass` (weakest-link: any `self-authored` leaf makes the whole PR `self-authored`). Legacy envelopes signed before this field existed verify unchanged, defaulting to `self-authored` on both sides of the comparison.

**AISDLC-572 fix — role binding, not just timing.** As shipped in AISDLC-568, `independent` was credited on timing alone: the marker never recorded which agent role fired the `SubagentStart` hook, so ANY subagent dispatched within the window (a `developer`, a `rebase-resolver`, anything) made a leaf `independent`. A coordinator that dispatched a `developer` subagent (which writes a marker) and then hand-authored a `code-reviewer` transcript within the window got `independent` with no fabrication — an unrelated real marker was silently reused. AISDLC-572 closes this by (a) having the hook also record `agent_type` from the wire payload (it was already present on every `SubagentStart` event; the hook simply never read it) and (b) requiring the marker's `agentType` to be in the reviewer allowlist BEFORE the timing check runs. The lookback window was narrowed from 2 hours to 30 minutes now that role-binding, not timing, is the primary defense.

**Honest limit of `verdictClass`.** This is a same-process/session HEURISTIC, not a cryptographic proof. On a single machine with a coordinator that already has full repo Bash/Write access, a sufficiently determined coordinator could fabricate a marker file directly — including forging a reviewer `agentType` string — bypassing the need to actually spawn a subagent. What this DOES defend against is the common, non-adversarial case: a coordinator running the reviewer's prescribed Bash steps itself (in the same conversation turn), or reusing an unrelated real marker from a non-reviewer subagent, rather than dispatching a real reviewer `Agent`/`Task` subagent — that case now correctly downgrades to `self-authored`. Closing the adversarial-coordinator gap (a coordinator willing to forge `agent_type` itself) requires binding the leaf to the reviewer subagent's own harness-captured execution transcript (DEC-0012 opt-a) — see `harnessTranscriptHash` below.

**`harnessTranscriptHash` (AISDLC-570, DEC-0012 opt-a, DEC-0013 → opt1 trust model, shipped).** Every v6 transcript leaf may additionally carry an optional `harnessTranscriptHash` field: the SHA-256 of the reviewer subagent's own harness-captured execution transcript (Claude Code writes every subagent invocation to `~/.claude/projects/<project-slug>/<session-id>/subagents/agent-<agent-id>.jsonl` — a file the coordinator did not author and cannot cheaply fabricate without actually paying for a real `Agent`/`Task` tool call). `cli-attestation emit-leaf` resolves the matching `SubagentStart` marker (same marker as `verdictClass`, read-only — it does not consume it), locates the harness transcript, confirms the marker/harness-reported `agentType` is a reviewer role (`code-reviewer`, `test-reviewer`, `security-reviewer`, or their `-codex` variants — never `developer`), and confirms the transcript contains the diff-binding nonce literal (`[[ai-sdlc-nonce: <nonce>]]`, embedded in the reviewer's dispatch prompt via an explicit `--nonce` passed to `emit-leaf`) BEFORE setting the hash — any failure at any step fails closed to `null`, never over-claiming. See `pipeline-cli/src/attestation/harness-transcript.ts` for the full mechanism.

**DEC-0013's trust-model resolution: sign-time-only, informational.** Unlike every other v6 field, `harnessTranscriptHash` is verified ONCE, at sign time, on the operator's own machine, immediately after the reviewer ran — while the ephemeral `~/.claude/projects/**` transcript still exists. A downstream CI verifier CANNOT independently re-derive it (a fresh CI runner has no `~/.claude/projects/` at all). Once signed, however, the value IS part of the leaf that gets Merkle-hashed and root-signed, so post-sign tampering with a declared value is still caught by the root signature — the verifier simply cannot re-compute it from scratch the way it can `transcriptHash`. This is a genuine, disclosed departure from the rest of RFC-0042's "CI can independently re-verify" design property, chosen because the alternative (committing the harness transcript itself to the repo) trades a real repo-hygiene / secrets-exposure risk (full reviewer prompts + tool outputs) for CI-side re-derivability that a spot-check operator tool can provide on demand instead. See `docs/design/aisdlc-570-opt-a-feasibility.md` §4.4-§the escalated decision for the full analysis of the three options considered.

**Honest limit of `harnessTranscriptHash`.** Proves a real, harness-dispatched subagent invocation (an actual `Agent`/`Task` tool call) produced the transcript for a reviewer-typed role, once the diff-binding nonce is embedded in its prompt. Does NOT prove: the subagent's judgment was free of coordinator-engineered prompt bias (a coordinator willing to pay the LLM-token cost can still spawn a real reviewer subagent with a rubber-stamp-biased prompt — not closeable by any transcript-binding mechanism); reviewer identity beyond the `agentType` string the coordinator itself selected at spawn time. **End-to-end wiring status: ACTIVE (AISDLC-573).** Both canonical reviewer-dispatch paths — `/ai-sdlc execute` Step 7b and `/ai-sdlc orchestrator-tick`'s reconcile-step reviewer fan-out (Step 3) — now generate a per-PR/per-pass nonce via `cli-attestation generate-nonce` BEFORE spawning any reviewer, embed the literal (via `cli-attestation nonce-marker`) in EACH reviewer's prompt, and pass the SAME value to every `emit-leaf --nonce` call for that PR. `harnessTranscriptHash` therefore now populates for real reviewer runs on both paths. Session-id resolution still uses the disclosed most-recently-modified-directory fallback (no reliable env-var source for a running session's own id was found — see `docs/design/aisdlc-570-opt-a-feasibility.md` §4.3 opt-i); `--claude-session-id` is not passed by either dispatch path. The honest limits above are unchanged by activation — this closes the wiring gap, not the trust-model boundary.
