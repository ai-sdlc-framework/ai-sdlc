...
**`verdictClass` (AISDLC-568, DEC-0012 opt-b, shipped).** Every v6 transcript leaf now carries a `verdictClass: 'independent' | 'self-authored'` field. The signal: Claude Code's `SubagentStart` hook (`ai-sdlc-plugin/hooks/subagent-start.js`) fires ONLY when the harness itself dispatches a subagent via the `Agent`/`Task` tool — it cannot be triggered by a coordinator merely writing files or emitting text. On each firing, the hook writes a marker file to `.ai-sdlc/subagent-sessions/<agent-id>.json`. `cli-attestation emit-leaf` checks for an unconsumed marker whose firing time is within a bounded window (2h) of the transcript's last-write time; a match classifies the leaf `independent` (and consumes the marker so it cannot back-stop a second leaf); no match (or any error/staleness) classifies it `self-authored` — the fail-safe default. The verifier (`verifyV6Envelope`) cross-checks the envelope's declared `verdictClass` against the on-disk leaf's Merkle-proved value (rejecting a mismatch as tampering) and surfaces a per-leaf breakdown plus an `overallVerdictClass` (weakest-link: any `self-authored` leaf makes the whole PR `self-authored`). Legacy envelopes signed before this field existed verify unchanged, defaulting to `self-authored` on both sides of the comparison.

**Honest limit of `verdictClass`.** This is a same-process/session HEURISTIC, not a cryptographic proof. On a single machine with a coordinator that already has full repo Bash/Write access, a sufficiently determined coordinator could fabricate a marker file directly, bypassing the need to actually spawn a subagent — the marker file is just another file the coordinator can write. What this DOES defend against is the common, non-adversarial case: a coordinator running the reviewer's prescribed Bash steps itself (in the same conversation turn) rather than dispatching a real `Agent`/`Task` subagent — that case produces NO `SubagentStart` firing and therefore correctly downgrades to `self-authored`. Closing the adversarial-coordinator gap requires binding the leaf to the reviewer subagent's own harness-captured execution transcript (DEC-0012 opt-a) — tracked as a harness-coupled follow-up, not implemented here.
