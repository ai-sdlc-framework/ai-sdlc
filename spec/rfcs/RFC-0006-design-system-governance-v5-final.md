...
**Status:** Final v5.1 — Token Versioning, Multi-Brand Inheritance, Cross-Platform, Adapter Priority Decisions; OQ-7 + OQ-8 closed by operator walkthrough (2026-08-13, full rigor rubric). OQ-7: automated autonomy-threshold calibration is tooling, not spec — bound to the framework's corpus-aggregator + promotion-runbook pattern and scheduled behind the design-metrics wiring. OQ-8: closed by escalation — RFC-0008 §4 (Design Intent Document) is Final and RFC-0009 is Implemented; a cross-reference now makes that resolution findable from this document. **§18 has 0 open questions.**
**Lifecycle:** Implemented
**Created:** 2026-03-28
**Revised:** 2026-04-13
**Authors:** [Author Name]
...
| v5.1 | 2026-08-13 | Operator OQ walkthrough (full rigor rubric) closed the final two open questions; §18 now has 0 open. **OQ-7** (autonomy threshold calibration): resolved as tooling-not-spec and bound normatively in §13.2 to the framework's corpus-aggregator + promotion-runbook pattern (eleven existing runbooks), with implementation scheduled behind the §A.10 design-metrics wiring and manual selection within `calibrationRange` remaining conformant meanwhile. **OQ-8** (soul purpose document): closed by escalation — RFC-0008 §4 (Design Intent Document, Final) and RFC-0009 (Implemented, wired) resolve it; a light see-also pointer was added under §1 because this RFC previously contained zero references to the artifact answering its own OQ. No normative content changed; no reciprocal frontmatter dependency added (RFC-0008 already requires RFC-0006 — a cycle). Three non-substantive corrections were also required before the upstream-OQ gate would clear, none of which resolve anything new: **OQ-3** carried only "Escalated to RFC-0007" with no machine-readable marker — RFC-0007 has since shipped as `status: Final`, so a `**Resolution:**` marker was added; **OQ-6** was resolved in v4 but its `**Resolved.**` marker is period-terminated, which the upstream-OQ parser does not match (it requires `:` or whitespace after the word), so the punctuation was corrected — the blind spot is isolated to this RFC (1 occurrence repo-wide, so no framework fix is warranted); and **§9.5** still described RFC-0007 as "not yet written", which was stale. Net effect: tasks citing RFC-0006 no longer need a `blocked.reason` override to clear the DoR gate. |
...
> **See also: [RFC-0008](RFC-0008-ppa-triad-integration-final-combined.md) §4 — the Design Intent Document (DID).** This RFC governs the design *system* (tokens, catalog, visual regression). It deliberately does NOT define the authoritative *design intent* artifact — what the product should feel like — which both this RFC and the Product Priority Algorithm need to reference. RFC-0008 §4 defines the DID as that shared artifact and specifies its ownership (split-authority, §A1) and its relationship to `DesignSystemBinding`; [RFC-0009](RFC-0009-tessellated-design-intent-documents.md) extends it to multi-Soul platforms and is Implemented. Read those before assuming design intent lives here — maintaining a second copy in this document is the divergence risk §18 OQ-8 was raised to prevent.
...
Those concerns belong to [RFC-0007](RFC-0007-figma-make-pipeline-integration-v1-final.md) (Figma Make pipeline integration — `status: Final` as of this writing; the "not yet written" note here was stale and was corrected 2026-08-13). The boundary is: this adapter reads tokens; RFC-0007 reads designs. These adapters MUST NOT overlap in their Figma API surface. If a Figma API call could be attributed to either scope, it belongs in RFC-0007.
...
**Calibration mode — normative binding (OQ-7 resolution, 2026-08-13).** That SHOULD is satisfied by the framework's established **corpus-aggregator + promotion-runbook** pattern rather than by a design-system-specific mechanism: an aggregator reads an append-only metrics corpus and emits a recommendation envelope (`safe-to-promote` / `continue-soak` / `insufficient-data`) with operator-tunable floors, and a matching `docs/operations/*-promotion.md` runbook documents the hybrid path. Eleven promotion runbooks already ship on this pattern (orchestrator, deps-composition, DoR, estimation, decision-catalog, signal-ingestion, variant-pattern, embedding-substrate, adopter-authoring, operator-TUI, untrusted-PR-gate). Design-system thresholds adopt the same shape; the implementation is **not** a spec obligation and is scheduled separately, because its input — the six `DesignMetrics` of §A.10 — is not yet reachable from a package barrel (see §A.10 and the design-review architecture concepts page). Until it ships, teams select values within each `calibrationRange` manually, which remains conformant.
...
6. ~~**Design review tooling**~~ **Resolution:** resolved in v4 (see revision history); marker punctuation corrected 2026-08-13 so the upstream-OQ parser recognises it. No substantive change. The design review gate's structured feedback interface (§8.5.3) is surfaced through two complementary channels:
...
7. ~~**Autonomy threshold calibration**~~ **Partially resolved in v4.** §13.2 now frames all thresholds as calibration templates with explicit `calibrationRange` fields. The remaining question is whether the orchestrator should provide an automated calibration mode that observes pipeline operations during Phase 1 and recommends initial thresholds based on observed data — rather than requiring teams to manually select values within the calibration range. This is a tooling decision, not a spec decision, and MAY be addressed in the orchestrator implementation rather than the specification.
   ...
   **Resolution (2026-08-13, full rubric):** **Tooling, not spec — the SHOULD is bound to the framework's existing corpus-aggregator + promotion-runbook pattern, and the implementation is scheduled behind the design-metrics wiring.** Industry research: observe-and-recommend is a standard *separate mode* — Kubernetes VPA ships a recommender that emits recommendations without acting (`updateMode: "Off"`), Google's SRE workbook sets SLO targets from observed baseline rather than chosen numbers, and Argo Rollouts' `AnalysisTemplate` evaluates a canary against a baseline window before promotion; ratchet-baselines (ESLint, type-coverage) record current state and fail only on regression. **Internal precedent is decisive:** eleven `docs/operations/*-promotion.md` runbooks already implement exactly this, with aggregators emitting `safe-to-promote` / `continue-soak` / `insufficient-data` and operator-tunable floors (`--min-tasks`, `--unattended-threshold`). **Substantive gap surfaced by the walkthrough:** the six `DesignMetrics` (§A.10) that any design-threshold calibrator must consume are implemented and unit-tested but exported by nothing and imported by nothing — building the consumer first would ship it as dead code too. **Refinement over v4:** §13.2 now names the satisfying pattern normatively instead of leaving an unbound SHOULD, and states that manual selection within `calibrationRange` remains conformant until the calibrator ships. **Counter-argument:** "This restates the status quo and leaves an unkept promise — either delete the SHOULD or build it now." Rebuttal: the resolution differs from the status quo in three ways — it binds the SHOULD to a named, shipped pattern, it schedules the work as a dependency-ordered task rather than an aspiration, and it records why it cannot be built yet. **Selected over building it immediately** because its only input is unreachable code; **over deleting the SHOULD** because eleven runbooks show the pattern earns its keep; **over a generalised cross-RFC calibration substrate** because that is premature abstraction against this project's stated preference for composition.
...
8. **Soul purpose document and PPA Triad integration — Escalated to RFC-0008.** The PPA Triad Integration Analysis identifies a foundational gap: RFC-0006 governs the design system, PPA governs product priority, but neither document defines the authoritative *design intent artifact* that both need to reference. PPA's Sα₂ (Vibe Coherence) assesses alignment against "brand and UX guidelines" maintained independently from the `DesignSystemBinding`, creating a duplication problem that will become a divergence problem. Additionally, five integration connections between the PPA scoring model and RFC-0006's `DesignSystemBinding` resource are specified in the Triad Analysis but are out of scope for this RFC. These include: design system health as a product prioritization input (Eρ₄), design authority as a formal human curve channel (HC_design), and delivery risk adjustment based on code area quality metrics (Dπ₁). RFC-0008 addresses all five connections and resolves the soul purpose document ownership question as a prerequisite.
   ...
   **Resolution (2026-08-13, full rubric):** **Closed by escalation — the escalation target shipped, and this RFC now carries a pointer so the resolution is findable from here.** Verification rather than research: RFC-0008 is `status: Final` and its §1 states verbatim that it "resolves a foundational gap identified in design leadership review: neither PPA v1.0 nor RFC-0006 defines the authoritative *design intent artifact* that both systems need to reference. Section 4 introduces the Design Intent Document as that shared artifact and specifies its ownership, structure, and relationship to the `DesignSystemBinding` resource." Its revision history records DID split-authority ownership (A1), the Eρ₄ lifecycle escape valve (A3), and HC_design weighting (A4); Connection 2 (`DesignSystemBinding` → Eρ₄) and Connection 3 (quality metrics → Dπ₁) have dedicated sections. [RFC-0009](RFC-0009-tessellated-design-intent-documents.md) extends the DID to multi-Soul platforms and is `lifecycle: Implemented` across eight phases, and the code path is live — `sa-scoring/did-compiler` is imported by the orchestrator's public barrel and by both scoring layers, so this is not shelfware. **Substantive gap surfaced by the walkthrough:** RFC-0006 contained **zero** references to the Design Intent Document, RFC-0008 §4, or RFC-0009 — a reader of the governing design-system RFC had no path to the artifact that answers its own open question, which is the documentation form of exactly the divergence this OQ predicted. **Refinement:** a light cross-reference now sits under §1 (pointer only — the normative content stays in RFC-0008 §4). **Counter-argument:** "Editing a Final, tri-pillar-signed document invalidates that sign-off." Rebuttal: the pointer changes no normative content and is recorded as a revision-history row, this project's convention for post-sign-off refinement; an edit that changed normative content would warrant re-sign-off and is deliberately avoided here. **Selected over recording the resolution without the pointer** because an unfindable resolution is how the original divergence started; **over marking it Superseded** because that vocabulary applies to whole RFCs and the question was answered, not obsoleted; **over re-specifying the DID↔`DesignSystemBinding` relationship inside this RFC** because RFC-0008 §4 already specifies it and a second copy would recreate the duplication this OQ exists to prevent. **A reciprocal `assumes:` frontmatter entry was deliberately NOT added — RFC-0008 already declares `requires: RFC-0006`, so the pair would form a dependency cycle.**
