...
| 0048 | Per-Repo Configurable Governance Hard-Rules                                                          | Draft        | Draft            | 5   | Dominique Legault                                  | [RFC-0048-per-repo-configurable-governance.md](RFC-0048-per-repo-configurable-governance.md)           | assumes RFC-0043. The plugin hard-codes governance hard-rules (NEVER merge PRs / force-push / close) into every session + subagent + `execute` command, while the enforcement substrate (`agent-role.yaml` `blockedActions`) is already per-repo — the two have drifted (`git merge*` doesn't match `gh pr merge`). Makes hard-rules one per-repo policy source of truth in `agent-role.yaml`, rendered into every governance surface, **defaults strict**. Trust boundary follows work-item tier (established operator architecture): internal backlog tasks (higher trust) may auto-merge on green+CLEAN; external GitHub-sourced work (lower trust) never auto-merges. 5 OQs pending operator walkthrough. Trigger: local-trades adopter (plugin 0.19.0); tasks AISDLC-601/602. |
