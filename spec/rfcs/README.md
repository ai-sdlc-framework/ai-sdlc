...
| 0024 | Emergent Issue Capture + Triage Pattern                                                              | Draft       | Implemented  | 0   | dominique@reliablegenius.io                                  | [RFC-0024-emergent-issue-capture-and-triage.md](RFC-0024-emergent-issue-capture-and-triage.md) | requires RFC-0011, RFC-0015. Sidecar mechanism for capturing findings mid-work + triage rubric + decision-deferred handoff. Addresses VISION.md §5 emergent-work gap. **Fully implemented (AISDLC-269, PR #483 2026-05-15)** — `cli-capture` ships §5.1-§5.4 capture paths; `spec/schemas/capture-record.v1.schema.json` formalizes §6 schema; `captures-pending.ts` filter implements §9.3 pre-dispatch guard. All 12 OQs resolved 2026-05-15 (§15 Resolutions); lifecycle defaults captured in §15.1 (timebox + default-on-silence convention; per-org configurable). |
