...
| 0019 | Embedding Provider Adapter Framework                                                                 | Draft       | Ready for Review | 0 | dominique@reliablegenius.io                                  | [RFC-0019-embedding-provider-adapter.md](RFC-0019-embedding-provider-adapter.md)  | Adapter framework for text→vector embedding providers; OpenAI text-embedding-3-small ships as default; adopters can plug custom adapters per the harness-adapter pattern (RFC-0010 §13). **OQ re-walkthrough 2026-05-21 with full rigor rubric** (problem statement → industry research → 3-4 options with tradeoffs → recommendation + counter-argument per OQ) — replaces 2026-05-16 first-pass resolutions flagged as too shallow. Refinements: (OQ-1) scale-escalation heuristic (>100K entries OR p95 read >250ms → swap JSONL→sqlite); (OQ-2) per-consumer `staleVectorPolicy` API parameter added — RFC-0009 Eτ drift pins fail-loud at API site; (OQ-3) **split** strict no-op cross-PROVIDER + cross-VERSION delegates to OQ-2 (resolves logical conflict in v0.2); (OQ-4) three-layer precedence (framework → adapter-declared → per-org) + catalog dedup at 1/7/30/60/89-day milestones; (OQ-5) explicit `spec/schemas/` placement for both schemas; (OQ-6) per-consumer `consumerLabel` cost-tracker dimension; (OQ-7) per-adapter `billingModel` + `unified-cost-report` view. §15.1 schema updated. Phase tasks AISDLC-337..340 amended; AISDLC-341 unchanged. **Unblocks RFC-0009 Phase 4.2 Eτ rule #2** (embedding-distance drift detection). |
