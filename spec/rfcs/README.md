...
| 0006 | Design System Governance Pipeline                                                                    | Final       | Implemented  | 0   | Dominique Legault, Morgan Hirtle, Alexander Kline             | [RFC-0006-design-system-governance-v5-final.md](RFC-0006-design-system-governance-v5-final.md) | requiresDocs: tutorial, operator-runbook, api-reference. Design locked. **All OQs resolved 2026-08-13 (operator walkthrough, full rigor rubric)** — §18 now 0 open. OQ-7 (autonomy threshold calibration) resolved as **tooling, not spec**: §13.2's unbound `SHOULD` is now bound normatively to the framework's corpus-aggregator + promotion-runbook pattern (the eleven existing `docs/operations/*-promotion.md` paths), with the design-specific calibrator scheduled behind the §A.10 design-metrics wiring and manual selection within each `calibrationRange` remaining conformant until then. OQ-8 (soul purpose document) **closed by escalation** — RFC-0008 §4 defines the Design Intent Document as the shared artifact (Final) and RFC-0009 extends it to multi-Soul platforms (Implemented, code path wired via `sa-scoring/did-compiler`). The walkthrough surfaced that this RFC contained **zero** references to that artifact, so a light see-also pointer was added under §1; no reciprocal `assumes:` was added because RFC-0008 already declares `requires: RFC-0006` (cycle). Addendum A's four-layer design review architecture is documented at `docs/concepts/design-review-architecture.md`. |
...
| 0018 | In-Soul Journey Pattern                                                                              | Draft       | Ready for Review | 0 | Morgan Hirtle                                                | [RFC-0018-in-soul-journey-pattern.md](RFC-0018-in-soul-journey-pattern.md)        | Carved out of RFC-0009 per OQ-3. Journey = a temporally-ordered user flow within a soul that carries distinct design intent and completion criteria. Practitioner validation via InternalAdopter accessibility audit pipeline. **OQ walkthrough complete 2026-05-28 with full rigor rubric** — all 10 §10 OQs resolved (per-org journey count limits 10/50; per-org state cardinality 12/100 + concrete v1 workaround message; schema-enforced flat / no sub-journeys / Decision auto-promote on ≥2 requests; closed completion-criteria enum + future CEL pre-recommendation; operator-supplied MetricSnapshot + stale-metric Decision + v2 MetricsAdapter trajectory; graduated Eρ₅ degradation + threshold Decisions + per-Soul auditOverdueGracePolicy; additive WCAG enum + WCAG 3.0 scoring-model preview + superseded advisory; AST scan from v1 reusing RFC-0009 §13 Rule #1 infrastructure; cross-soul journeys documented limitation + concrete per-soul-with-handoff workaround; 4th rule in same §13 engine + Tessellation§13RuleRegistry registration mechanism). §10.1 codifies `.ai-sdlc/journey-config.yaml` per-org schema. Implementation broken into 6 phase tasks AISDLC-465..470. Engineering + Operator sign-off (Dominique) added. requires RFC-0009, RFC-0017, RFC-0035. |
| 0019 | Embedding Provider Adapter Framework                                                                 | Draft       | Ready for Review | 0 | Dominique Legault                                  | [RFC-0019-embedding-provider-adapter.md](RFC-0019-embedding-provider-adapter.md)  | Adapter framework for text→vector embedding providers; OpenAI text-embedding-3-small ships as default; adopters can plug custom adapters per the harness-adapter pattern (RFC-0010 §13). **OQ re-walkthrough 2026-05-21 with full rigor rubric** (problem statement → industry research → 3-4 options with tradeoffs → recommendation + counter-argument per OQ) — replaces 2026-05-16 first-pass resolutions flagged as too shallow. Refinements: (OQ-1) scale-escalation heuristic (>100K entries OR p95 read >250ms → swap JSONL→sqlite); (OQ-2) per-consumer `staleVectorPolicy` API parameter added — RFC-0009 Eτ drift pins fail-loud at API site; (OQ-3) **split** strict no-op cross-PROVIDER + cross-VERSION delegates to OQ-2 (resolves logical conflict in v0.2); (OQ-4) three-layer precedence (framework → adapter-declared → per-org) + catalog dedup at 1/7/30/60/89-day milestones; (OQ-5) explicit `spec/schemas/` placement for both schemas; (OQ-6) per-consumer `consumerLabel` cost-tracker dimension; (OQ-7) per-adapter `billingModel` + `unified-cost-report` view. §15.1 schema updated. Phase tasks AISDLC-337..340 amended; AISDLC-341 unchanged. **Unblocks RFC-0009 Phase 4.2 Eτ rule #2** (embedding-distance drift detection). |
...
| 0022 | Compliance Posture + Audit Surface                                                                   | Draft       | Ready for Review | 0 | Dominique Legault                                  | [RFC-0022-compliance-posture-audit-surface.md](RFC-0022-compliance-posture-audit-surface.md) | Adopter declares regulatory posture (HIPAA/SOC2/PCI-DSS/GDPR/etc.); framework derives gate defaults (DB pool isolation, secret-scan strictness, attestation requirement, retention) and exports audit evidence packs. RFC-0020 and RFC-0021 carved out of RFC-0009 (OQ-7); RFC-0009 OQ-11 trigger checklist references RFC-0022 as the canonical regime-declaration surface. **OQ walkthrough complete 2026-05-16** — all 7 OQs resolved (in-tree YAML + adopter override in v1, forced attestedNotes with auto-fill, hand-curated control-feature-map + per-RFC reviewer check, single .tar.gz audit export, on-demand export only for v1, single posture for v1 with loader API designed for v2 multi-posture additive composition, PR template + reviewer-subagent check). §13.1 codifies per-org `.ai-sdlc/compliance.yaml` config schema. Implementation broken into 4 phase tasks AISDLC-322..325. |
| 0023 | Operator TUI — Pipeline Monitoring + Steering Surface                                                | Approved    | Signed Off   | 0   | Dominique Legault                                  | [RFC-0023-operator-tui-pipeline-monitoring.md](RFC-0023-operator-tui-pipeline-monitoring.md) | requires RFC-0014, RFC-0015. Operator-facing terminal interface to monitor + unblock the autonomous pipeline. All 10 OQs resolved via operator walkthrough 2026-05-03. Lifecycle audit 2026-05-13 promoted Ready for Review → Signed Off: AISDLC-178 umbrella + all 7 phases 178.1–178.7 + extension 178.4.1 shipped. Flag default-on follows the same opt-in pattern as RFC-0014 / RFC-0015. |
| 0024 | Emergent Issue Capture + Triage Pattern                                                              | Draft       | Ready for Review | 0 | Dominique Legault                                  | [RFC-0024-emergent-issue-capture-and-triage.md](RFC-0024-emergent-issue-capture-and-triage.md) | requires RFC-0011, RFC-0015. Sidecar mechanism for capturing findings mid-work + triage rubric + decision-deferred handoff. Addresses VISION.md §5 emergent-work gap. **Partial implementation 2026-05-13** — AISDLC-269 / PR #483 shipped `cli-capture` (§5.1-§5.4), `spec/schemas/capture-record.v1.schema.json`, `captures-pending.ts` filter against the 2026-05-13 first-pass OQ resolutions. **2026-05-15 walkthrough revised OQ-1/2/3/5/7/9/11 + added §15.1 Capture Lifecycle Defaults** — lifecycle rolled back from `Implemented` to `Ready for Review`. Gap closed by **RFC-0024 Refit (AISDLC-320 / 321 + 275-278)**; lifecycle flips back to `Implemented` after Refit Phase 6 (AISDLC-278) ships. |
| 0025 | Framework Quality Monitoring (Non-Decision Failure Modes)                                            | Draft       | Ready for Review | 0 | Dominique Legault                                  | [RFC-0025-framework-quality-monitoring.md](RFC-0025-framework-quality-monitoring.md) | requires RFC-0015, RFC-0024. Distinguishes "operator under-decided" failures (fix the issue) from "framework misbehaved" failures (fix the framework); auto-routes the latter into bugfix backlog with severity scoring; closes the AISDLC-176-style "valid commit stranded" loop. Operationalizes VISION.md §4 honest failure modes. **OQ walkthrough complete 2026-05-15** — all 10 OQs resolved in §13 (confidence-bucketed classifier, multi-window recurrence, per-org-configurable suggest-only attribution, operator-initiated pre-filled GitHub issue for upstream reporting, capture-record-based coverage-gap response, composite blast-radius determinism sampling, first-capture MTTR, instrumented operator-time-cost, strict vendor-namespace enforcement). §13.1 codifies the per-org config schema. **Partial impl 2026-05-13** — `tui/analytics/quality-reader.ts` + 9 playbook handlers ship. **AISDLC-270 / PR #481 paused** — pre-walkthrough impl candidate; audit against the resolved OQs pending (AISDLC-300 sweep) before unblock/refit/rebuild decision. |
| 0026 | Exploration Workstream Pattern                                                                       | Draft       | Draft        | 12  | Dominique Legault                                  | [RFC-0026-exploration-workstream-pattern.md](RFC-0026-exploration-workstream-pattern.md) | requires RFC-0011, RFC-0015, RFC-0024. First-class "spike/research" workstream type that bypasses DoR's decision-frontloading gate (since the goal IS to discover the unknowns); explicit time-box + handoff back to standard execution flow when knowns crystallize. Addresses VISION.md §5 exploration-mode gap. |
...
| 0028 | Engineering-Axis Substrate Enforcement for Multi-Soul Platforms                                      | Draft       | Ready for Review | 0 | Alexander Kline                                              | [RFC-0028-engineering-axis-substrate-enforcement.md](RFC-0028-engineering-axis-substrate-enforcement.md) | requires RFC-0008, RFC-0009, RFC-0035. Substrate Contract pattern: which fields are `core` (locked, identity-class) vs `evolving` (override-permitted) at substrate level for multi-soul platforms. **OQ walkthrough complete 2026-05-27 with full rigor rubric** — all 4 §7 OQs resolved. Resolutions: (OQ-7.1) field-level `identityClass` with canonical taxonomy (compliance locks / regime / director / `complianceFloor: inherit` = `core`; operational cadence / scoring weights / similarity thresholds / quotas = `evolving`; default `core` for novel fields) — harmonizes with shipped discriminant in `orchestrator/src/sa-scoring/layer1-deterministic.ts`; (OQ-7.2) canonical pairing of structural (CI BLOCKS PR) + statistical (G0 SURFACES) drift detection with cold-start calibration window; (OQ-7.3) normative preservation of centroid slot via §3.2 named-consumer rule; (OQ-7.4) light cross-refs added in RFC-0009 §5.2 AND §7.2 pointing at RFC-0028. Implementation broken into 5 phase tasks AISDLC-452..451. |
...
| 0030 | Signal Ingestion Pipeline (Demand Sources → D1)                                                      | Draft       | Ready for Review | 0 | Alexander Kline                                              | [RFC-0030-signal-ingestion-pipeline.md](RFC-0030-signal-ingestion-pipeline.md)    | requires RFC-0005, RFC-0008, RFC-0011, RFC-0019, RFC-0022, RFC-0024, RFC-0025, RFC-0029, RFC-0035. Adapter framework for ingesting demand signals (interviews, tickets, telemetry, ICP cohort data) into PPA's D1 channel. **OQ re-walkthrough 2026-05-26 with full rigor rubric** — replaces 2026-05-16 first-pass resolutions. Refinements on every OQ: (OQ-13.1) env-var-only v1 adapter scope + dual Decision routing (`adapter-credential-not-configured` vs `adapter-credential-rejected`); (OQ-13.2) per-org `acceptedLanguages` config + explicit `franc` language detection + documented BM25 multi-language quality degradation; (OQ-13.3) **per-stage residency enforcement points** (fetchSignals, clustering, storage, unified-report) + multi-posture UNION composition; (OQ-13.4) per-operator rate limit (default 10/day) + optional `evidenceUrl` field + manual-share quality metric (`Decision: manual-signal-share-elevated` at >30%); (OQ-13.5) z-score flooding detection on 7d baseline + quarantine + operator one-click unquarantine + reputation-weighting deferred to v2. §11 config schema expanded. Original phase tasks AISDLC-343..348 shipped; refinements land via follow-up tasks AISDLC-430..434. |
...
| 0034 | PR Merge Critical-Path Ordering                                                                      | Reserved    | Placeholder  | —   | Dominique Legault                                  | (none yet — reservation only; AISDLC-178.4.1 ships the minimum derivation in RFC-0023 §7.2) | Longer-term home for auto-rebase trigger semantics, `depends-on` label conventions (canonical syntax + body-marker spec), and multi-repo PR ordering. AISDLC-178.4.1 reserved this number per its AC #8 (originally numbered RFC-0028 in the task spec; that slot was already taken by Engineering-Axis Substrate Enforcement, so this is the actual reservation). requires RFC-0014, RFC-0023. |
| 0035 | Decision Catalog and Operator Decision Routing                                                       | Draft       | Ready for Review | 0 | Dominique Legault                              | [RFC-0035-decision-catalog-operator-routing.md](RFC-0035-decision-catalog-operator-routing.md) | requires RFC-0011, RFC-0023, RFC-0024, RFC-0029. Operationalizes VISION.md §3 (operator-as-decision-steward): event-sourced `Decision` resource + deterministic-first ladder + auto-apply-with-override-window + RFC-0014/0016 composition. **OQ walkthrough complete 2026-05-15** — all 14 OQs resolved per §15 Resolution markers; §15.1 codifies the 8 design patterns. Lifecycle promoted Draft → Ready for Review; awaits per-owner sign-off. |
| 0036 | Spec-Kit Bridge and Adopter Spec-Authoring Surface                                                   | Draft       | Ready for Review | 0 | Dominique Legault                                  | [RFC-0036-spec-kit-bridge-adopter-authoring.md](RFC-0036-spec-kit-bridge-adopter-authoring.md) | requires RFC-0010, RFC-0011, RFC-0024, RFC-0025, RFC-0035. Bridges [GitHub Spec Kit](https://github.com/github/spec-kit) (front-of-funnel artifact authoring) with ai-sdlc (back-of-funnel execution + governance). Defines the three-tier adopter authoring model (RFC → Spec → Task), the spec-kit import path (`ai-sdlc import-spec`), an optional adopter RFC scaffold (`ai-sdlc rfc init`), and the positioning updates that operationalize the AISDLC-248 repositioning. **OQ walkthrough complete 2026-05-16** — all 12 OQs resolved with cross-cutting **catalog-routing framing** (RFC-0035 G0 non-blocking contract): tasks.md only no fallback (OQ-1), drift via Decision auto-classify + 24h window (OQ-2), strict DoR at import (OQ-3), per-org config (OQ-4), one template (OQ-5), single BYO translator pattern (OQ-6), full DoR + analyze-metadata auto-resolve (OQ-7), constitution drift via catalog (OQ-8), Decision Engine primary positioning (OQ-9), refuse-and-emit-clarification on DoR fail (OQ-10), auto-detect-schema-refuse-unknown (OQ-11), dual CLI+slash surface (OQ-12). §14.1 codifies per-org `.ai-sdlc/adopter-authoring.yaml` schema. Implementation broken into 11 phase tasks AISDLC-326..336. |
| 0041 | Conductor / Worker Process Architecture for Autonomous Dispatch                                      | Draft       | Draft        | 0   | Dominique Legault                                  | [RFC-0041-conductor-worker-process-architecture.md](RFC-0041-conductor-worker-process-architecture.md) | requires RFC-0010, RFC-0012, RFC-0015. Names the Conductor (interactive Claude Code session) + Worker (separate `claude -p` subprocess) split that RFC-0015 implied. Closes the 2026-05-20 "in-CC dispatch" gap where Anthropic's 600s background-agent watchdog killed 6 of 7 dev subagents. Defines the filesystem-backed Dispatch Board protocol (queue/inflight/done/failed manifests) + a 150-LOC supervisor daemon. Backward-compatible: `/ai-sdlc execute` + shell-driven `cli-orchestrator tick` unchanged. |
| 0042 | Proof-of-Execution Attestation via In-Repo Merkle Transcripts                                        | Approved    | Implemented  | 2/2 | Dominique Legault                                  | [RFC-0042-proof-of-execution-attestation.md](RFC-0042-proof-of-execution-attestation.md)               | requires RFC-0011. Replaces the operator-key DSSE chain (AISDLC-380 + sub-attestation gate) with proof-of-execution: reviewer subagents capture full transcripts locally, leaves hash into an append-only Merkle index, operator signs root periodically, CI verifies inclusion proofs + spot-checks. Eliminates 13 attestation-pipeline gates (chore-commit cycle, v3/v4/v5 rebase fragility, per-machine signing keys, fork-PR brokenness). Operator's framing 2026-05-20: "It's the LLM work that's the expensive part to do on CI not the attestation." Cuts PR push cycles 3-4 → 1-2. No external service dependency (Rekor / OpenTimestamps deferred to future opt-in). **Original OQ walkthrough complete 2026-05-20** — all 7 §Open Questions resolved per Resolution markers. **Retrospective OQ walkthrough complete 2026-05-28** — five additional OQs (OQ-8 through OQ-12) documented post-hoc capturing design decisions that emerged during implementation: AISDLC-421 per-patch-id leaf storage with `merge=binary` driver; AISDLC-419 attestation-only descendant relaxation; AISDLC-448 tree-equivalent modulo attestation relaxation; AISDLC-422 signer/verifier exclusion-list symmetry invariant; AISDLC-445 CI per-patch-id directory staging under fork-PR security. Implementation tracked under AISDLC-383 umbrella + 7 phase sub-tasks + post-walkthrough refinements in `implementedBy:` frontmatter. |
| 0043 | Untrusted-Contributor PR Verification — Zero-Trust Gate with OpenShell Sandbox Isolation             | Approved    | Signed Off       | 0 | Dominique Legault                                  | [RFC-0043-untrusted-contributor-pr-verification.md](RFC-0043-untrusted-contributor-pr-verification.md) | requires RFC-0042; assumes RFC-0011, RFC-0022; related RFC-0038, RFC-0039, RFC-0010. Zero-trust path for external/fork PRs: deterministic pre-LLM diff/AST gate (hard-blocks `.github/**`, deps, lockfiles, `.ai-sdlc/**`), NVIDIA OpenShell policy-enforced sandbox with credential withholding at the proxy layer + differential testing, prompt-injection-hardened 3-reviewer matrix, and clean-room RFC-0042 attestation decoupled from the untrusted-eval environment. **OQ walkthrough complete 2026-06-02 with full rigor rubric** — all 6 §Open Questions resolved. Resolutions: (OQ-1) static `trusted-reviewers.yaml` ONLY + periodic drift-detection workflow via RFC-0035 G0; (OQ-2) CI default + local opt-in (operator distinguished untrusted-PR cost surface from internal-dogfood AISDLC-353 strategy); (OQ-3) configurable defaults (10min/2 cores/4GB/deny network) + hard-abort + G0 Decision; (OQ-4) operator-key Merkle ONLY for v1 + future Decision auto-promote on ≥2 adopter requests (RFC-0042 §8 Sigstore deferral preserved); (OQ-5) Docker default + RFC-0022 regime override + Kata/gVisor/MicroVM opt-in; (OQ-6) current allowlist + explicit boundary principle + Decision-Catalog request hook. Engineering + Operator sign-off (Dominique) added. Implementation broken into 6 phase tasks AISDLC-497..502. |
| 0044 | Worktree-HEAD Parent-Repo Serialization                                                              | Draft       | Draft        | 4   | Alexander Kline                                    | [RFC-0044-worktree-head-parent-serialization.md](RFC-0044-worktree-head-parent-serialization.md)       | assumes RFC-0010, RFC-0037, RFC-0038, RFC-0039, RFC-0040, RFC-0041. Addresses parent-repo HEAD contamination under concurrent `isolation: "worktree"` dispatch (N>=4 empirical onset). Documents three options: Option A (HEAD lock), Option B (detach parent HEAD), Option C (operator concurrency gate, default cap 3). Recommends Option C as first-pass shipping behavior. Composes with RFC-0041 Dispatch Board for in-flight count; composes with RFC-0038 §10 subprocess-plugin path (Option C expressible as `OrchestratorPlugin.beforeRun` hook). GitHub issue #732. |
| 0045 | Adopter Health Reporting — Upstream Feedback Loop for Config-Audit Findings                          | Draft       | Draft        | 4   | Dominique Legault                                  | [RFC-0045-adopter-health-upstream-reporting.md](RFC-0045-adopter-health-upstream-reporting.md)         | assumes RFC-0025, RFC-0022. OPTIONAL, consent-gated channel carrying `ai-sdlc doctor` (AISDLC-578) config-health findings upstream so maintainers gain visibility into adopter misconfiguration (today: zero downstream→upstream signal; stars/forks measure nothing about install health). Extends RFC-0025's shipped anonymized, opt-in, operator-submitted pre-filled-issue pattern — NOT an auto telemetry beacon (explicitly rejected as default). 6 §Open Questions for operator walkthrough: consent model, cadence/trigger, transport, anti-DoS/dedup (load-bearing — un-deduped design self-DoSes the tracker), payload scope + anonymizer proof, default repoUrl. Split from AISDLC-578 (local doctor ships first) per 2026-09-05 operator decision. |
| 0046 | Attested Reviewer Independence                                                                       | Approved     | Signed Off       | 0   | Dominique Legault                                  | [RFC-0046-attested-reviewer-independence.md](RFC-0046-attested-reviewer-independence.md)               | requires RFC-0043; assumes RFC-0042. Models reviewer independence as a re-derivable tier `independenceTier: none \| attested \| isolated`. `isolated` (the load-bearing claim) runs reviewers inside RFC-0043's `SandboxDriver` + clean-room-signer boundary (isolation the coordinator can't reach) — replacing RFC-0042's forgeable same-machine `verdictClass=independent` heuristic, which is deprecated (legacy-read only, NOT amended). All 5 OQs resolved 2026-09-06 via operator rubric: threat boundary = determined same-machine coordinator; hybrid re-derivable anchor; generalize RFC-0043 substrate (not a 3rd heuristic patch — the AISDLC-587 gaps become the honest `attested` lower tier); new field deprecating `verdictClass`; opt-in default + configurable gate-agnostic required-tier policy. Trigger: AISDLC-587. |
| 0047 | Re-Derivable Isolated-Review Anchor                                                                  | Draft        | Signed Off       | 0   | Dominique Legault                                  | [RFC-0047-re-derivable-isolated-anchor.md](RFC-0047-re-derivable-isolated-anchor.md)                   | requires RFC-0043; assumes RFC-0042, RFC-0046. Mints the anchor mechanism RFC-0046's `isolated` tier depends on but did not spec to a verifier-checkable level (AISDLC-590 / PR #1021 shipped a self-asserted `deployment: 'ci'` anchor signed with the operator's own key — a 3-reviewer reconcile 2026-09-06 found it CRITICAL-forgeable by RFC-0046 OQ-1's same-machine coordinator; held). **All 5 OQs resolved 2026-09-06 via operator rubric:** the anchor is a **CI-only ed25519 key** (private half only in a protected GH Actions environment; pubkey marked `ci-only` in `trusted-reviewers.yaml`) so the security anchor is the *root signature by that key* while the verifier stays 100% offline (reuses the ed25519 spine — Sigstore/Rekor is the forward hardening path); `{runId, workflowRef, signerKeyId}` bound into the hashed leaf as **audit-only** (closes the 590 over-claim); an unanchored `isolated` claim **downgrades** with a recorded reason (integrity failures still reject); the CI job **commits the attestation-only envelope to the PR branch** (fixes the 590 discarded-claim MAJOR); scope split — RFC-0046 keeps the taxonomy + `none`/`attested` + `requiredTier` policy engine (591 ships now), RFC-0047 owns the `isolated` producer/verifier + a single capability gate (`requiredTier: isolated` unsatisfiable until this ships). Trigger: AISDLC-590. |

| 0048 | Per-Repo Configurable Governance Hard-Rules                                                          | Draft        | Ready for Review | 0   | Dominique Legault                                  | [RFC-0048-per-repo-configurable-governance.md](RFC-0048-per-repo-configurable-governance.md)           | assumes RFC-0043. The plugin hard-codes governance hard-rules (NEVER merge PRs / force-push / close) into every session + subagent + `execute` command, while the enforcement substrate (`agent-role.yaml` `blockedActions`) is already per-repo — the two have drifted (`git merge*` doesn't match `gh pr merge`). Makes hard-rules one per-repo policy source of truth (`spec.governance`), rendered into every governance surface, **defaults strict**. **All 5 OQs resolved 2026-09-07 via operator rubric:** (1) dedicated `governance` block with per-rule value types; (2) merge-relaxation gates on work-item `sourceKind` provenance (internal-backlog trusted → auto-merge-on-green-CLEAN eligible, external-GitHub untrusted → never), policy declaration read base-branch-only; (3) operational rules (merge/force-push/close/branch-delete/reset-hard) configurable, integrity rules (CI-skip tokens, `.ai-sdlc/attestations\|verdicts` edits, PR-tree governance relaxation) permanently fixed; (4) deterministic `merge-if-eligible` CLI helper owns the green+CLEAN+tier gate, hook blocks raw `gh pr merge`; (5) ship granular **+** `operator-trusted` preset now (`strict`=defaults). Phases: AISDLC-601 (schema+presets+resolver+hooks), 602 (execute+enforcement), 603 (helper). Trigger: local-trades adopter (plugin 0.19.0). |
**Next available number:** RFC-0049.

> **Registry-drift note (2026-05-20):** RFC-0037 (adopter-context-extension-points), RFC-0038 (adopter-defined-reviewer-extension-point), RFC-0039 (adopter-defined-pipeline-gate-extension), and RFC-0040 (ppa-triage-diagnostic-and-human-curve-signal-sources) exist on disk but were not added to this registry when shipped. RFC-0041 picks up the next on-disk-available slot (0041) rather than 0037 to avoid renumbering already-merged RFCs. Backfilling rows for 0037–0040 is tracked separately.
...
>
> **Totals:** 31 active RFCs audited (excluding template + reservation-only entries). **~76 unresolved OQs**, **~154 resolved**. RFC-0029 has no OQ section by design (vision doc; positions stated). 2026-05-13 retrofit closed 15 OQs across RFC-0002 / RFC-0003 / RFC-0004 / RFC-0005 / RFC-0013 against shipped implementation; 2026-05-15 operator walkthrough closed all 12 in RFC-0024 AND all 14 in RFC-0035 AND all 10 in RFC-0025; 2026-05-16 operator walkthrough closed all 5 in RFC-0031 (audit-affirmed; lifecycle stays Implemented) AND all 7 in RFC-0022 AND all 12 in RFC-0036 AND all 7 in RFC-0019 AND all 5 in RFC-0030 (lifecycles promoted Draft → Ready for Review). **2026-05-21 RFC-0019 re-walkthrough with full rigor rubric** (problem statement → industry research → 3-4 options with tradeoffs → recommendation + counter-argument per OQ) — replaces 2026-05-16 first-pass shallow resolutions ("skims over questions and recommends authors' recommendations"); OQ counts unchanged (was already 0 unresolved) but resolution rigor substantially improved with refinements on every OQ (see RFC-0019 registry row). **2026-05-26 RFC-0030 re-walkthrough with same full rigor rubric** — replaces 2026-05-16 first-pass shallow resolutions; OQ counts unchanged (was already 0 unresolved) but every OQ refined with substantive specification (v1 adapter scope, language detection mechanism, per-stage residency enforcement, manual-entry anti-gaming, z-score flooding detection); follow-up implementation refinements land via AISDLC-430..434 on top of already-shipped Phase 1-6 substrate (see RFC-0030 registry row). **2026-05-27 RFC-0028 OQ walkthrough with full rigor rubric** — all 4 §7 OQs resolved (lifecycle Draft → Ready for Review). Resolutions: field-level `identityClass` canonical taxonomy + canonical pairing of structural / statistical drift detection + normative centroid-slot preservation + light RFC-0009 cross-refs in §5.2 and §7.2. Implementation phase tasks AISDLC-452..456 filed. Closes 4 of the remaining unresolved OQs. **2026-05-28 RFC-0018 OQ walkthrough with full rigor rubric** — all 10 §10 OQs resolved (lifecycle Draft → Ready for Review). Resolutions: per-org configurable journey count + state cardinality limits; schema-enforced flat (no sub-journeys); closed completion-criteria enum + future CEL pre-recommendation; operator-supplied MetricSnapshot + future MetricsAdapter trajectory; graduated Eρ₅ accessibility cadence degradation; additive WCAG enum + WCAG 3.0 forward-compat; AST scan from v1 reusing RFC-0009 §13 Rule #1 infrastructure; cross-soul journey limitation + concrete per-soul-with-handoff workaround; 4th rule in same §13 engine + concrete `Tessellation§13RuleRegistry.register` registration mechanism. Implementation phase tasks AISDLC-465..470 filed. Closes 10 of the remaining unresolved OQs. **2026-05-28 RFC-0042 retrospective OQ walkthrough** — five additional OQs (OQ-8..OQ-12) documented post-hoc capturing design decisions that emerged during implementation (per-patch-id leaf storage + merge=binary driver + head-binding relaxations + exclusion-list symmetry invariant + CI per-patch-id staging). RFC was already Implemented; no new phase tasks filed (exempt per the rfc-oq-walkthrough skill). Existing implementer task references (AISDLC-419, 421, 422, 445, 448) cited in each Resolution block. Documentation-discipline pass, not new design work. **2026-06-02 RFC-0043 OQ walkthrough with full rigor rubric** — all 6 §Open Questions resolved (lifecycle Draft → Ready for Review). Resolutions: static `trusted-reviewers.yaml` ONLY + drift-detection workflow (OQ-1); CI default + local opt-in (OQ-2 — operator explicitly distinguished untrusted-PR cost surface from internal-dogfood AISDLC-353 strategy); configurable resource limits + hard-abort + G0 Decision (OQ-3); operator-key Merkle ONLY for v1 + future Decision auto-promote on ≥2 adopter requests (OQ-4 — RFC-0042 §8 Sigstore deferral preserved); Docker default + RFC-0022 regime override + Kata/gVisor/MicroVM opt-in (OQ-5); current allowlist + explicit boundary principle + Decision-Catalog request hook (OQ-6). Implementation phase tasks AISDLC-497..502 filed. Closes 6 of the remaining unresolved OQs. RFC-0036 introduced **cross-cutting catalog-routing framing** via RFC-0035 G0 non-blocking contract (added to RFC-0035 in PR #496) — every operator-impacting OQ resolution routes through the Decision Catalog with auto-resolution OR timeboxed default-on-silence; pipeline never halts. RFC-0019 resolution unblocks RFC-0009 Phase 4.2 Eτ rule #2 (embedding-distance drift detection). RFC-0024 was prematurely flipped to `Implemented` by AISDLC-269 against 2026-05-13 first-pass resolutions; the 2026-05-15 walkthrough revised 7/12 OQs and rolled the lifecycle back to `Ready for Review` pending the RFC-0024 Refit (AISDLC-320 / 321 + 275-278). Remaining residuals documented in *Design locked* below.
...
**RFC-0018 — In-Soul Journey Pattern** — *moved to Awaiting lifecycle promotion (10/10 OQs resolved 2026-05-28 with full rigor rubric)*
...
**RFC-0028 — Engineering-Axis Substrate Enforcement** — *moved to Awaiting lifecycle promotion (4/4 OQs resolved 2026-05-27 with full rigor rubric)*
...
**2026-08-13 — RFC-0006 OQ walkthrough (OQ-7, OQ-8).** Both remaining open questions closed with the full rigor
rubric (problem statement → industry research → 3-4 options with tradeoffs → recommendation + counter-argument,
one AskUserQuestion per OQ). OQ count 1 → 0; lifecycle unchanged (already `Implemented`). Two substantive gaps
surfaced that a shallow resolution would have missed: (a) the six `DesignMetrics` that any autonomy-threshold
calibrator must consume are implemented and unit-tested but exported and imported by nothing, so building the
calibrator first would have shipped a second dead module — the work is now scheduled behind that wiring;
(b) RFC-0006 contained zero references to the Design Intent Document, RFC-0008 §4, or RFC-0009, leaving a reader
of the governing design-system RFC with no path to the artifact resolving its own OQ-8 — the documentation form
of the very divergence that OQ predicted. See the RFC-0006 registry row above and the `Resolution (2026-08-13,
full rubric)` markers in §18 of the RFC body. Clearing the gate additionally required three
non-substantive corrections (no new decisions): OQ-3 had no machine-readable marker despite its escalation target
RFC-0007 having shipped `Final`; OQ-6's `**Resolved.**` marker is period-terminated, which the upstream-OQ parser
does not match (it requires `:` or whitespace) — a blind spot isolated to this RFC, 1 occurrence repo-wide; and §9.5
still called RFC-0007 "not yet written". **Net effect: tasks citing RFC-0006 no longer need a `blocked.reason`
override**, verified by filing AISDLC-553 without one and watching the DoR gate pass.
