...
| 0047 | Re-Derivable Isolated-Review Anchor                                                                  | Draft        | Draft            | 5   | Dominique Legault                                  | [RFC-0047-re-derivable-isolated-anchor.md](RFC-0047-re-derivable-isolated-anchor.md)                   | requires RFC-0043; assumes RFC-0042, RFC-0046. Mints the anchor mechanism RFC-0046's `isolated` tier depends on but did not spec to a verifier-checkable level. AISDLC-590 / PR #1021 shipped a self-asserted `provenance.deployment: 'ci'` anchor signed with the operator's own key and never re-derived by the verifier — a 3-reviewer reconcile (2026-09-06) found it CRITICAL-forgeable by exactly RFC-0046 OQ-1's determined same-machine coordinator (local `clean-room-sign --independence-tier isolated` mints a trusted `isolated` leaf). PR #1021 held; operator directed a dedicated follow-up RFC. Lead direction: GitHub-OIDC-re-derivable anchor bound into the signed leaf + verifier validation against GitHub JWKS (distinct from the operator key). 5 §Open Questions for operator walkthrough: anchor trust root (OIDC vs CI-only key vs Sigstore/Rekor), evidence bound into the leaf, offline vs online verification, envelope durability, migration/`requiredTier: isolated` unsatisfiability. Trigger: AISDLC-590. |
