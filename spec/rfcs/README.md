...
| 0019 | Embedding Provider Adapter Framework                                                                 | Draft       | Ready for Review | 0 | dominique@reliablegenius.io                                  | [RFC-0019-embedding-provider-adapter.md](RFC-0019-embedding-provider-adapter.md)  | Adapter framework for text→vector embedding providers; OpenAI text-embedding-3-small ships as default; adopters can plug custom adapters per the harness-adapter pattern (RFC-0010 §13). **OQ walkthrough complete 2026-05-16** — all 7 OQs resolved (JSONL storage; lazy-re-embed default + per-org fail-loud opt-in; explicit no-op for cross-provider; 90d deprecation grace with per-org override; orchestrator placement; embeddingTokens cost-tracker line item; separate from SubscriptionLedger). Catalog-routing framing applied: stale-vector / cross-provider / deprecation / cost-budget events all route through RFC-0035 G0 catalog. §15.1 codifies `.ai-sdlc/embedding-config.yaml` per-org schema. **Unblocks RFC-0009 Phase 4.2 Eτ rule #2** (embedding-distance drift detection). Implementation broken into 5 phase tasks AISDLC-337..341. |
