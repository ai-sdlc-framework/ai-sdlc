...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `.husky/pre-push` extended with the auto-sign block (positioned after existing coverage gate)
- [ ] #2 Hook detects per-worktree `.active-task` sentinel (per AISDLC-81); falls through gracefully when sentinel absent (chore PRs, ad-hoc commits)
- [ ] #3 Hook reads verdict file from agreed location (decide /tmp/ vs `<worktree>/.ai-sdlc/verdicts/` in the PR per Q1)
- [ ] #4 Hook is idempotent: skips if `.ai-sdlc/attestations/<head-sha>.dsse.json` already exists at current HEAD
- [ ] #5 Hook output makes the re-push requirement obvious (decide silent-amend vs explicit re-push UX)
- [ ] #6 /ai-sdlc execute slash command updated: Step 10 (sign attestation) becomes a no-op call to git push, since the hook handles it
- [ ] #7 CLAUDE.md updated under "Testing" or new "Hooks" section documenting the auto-sign behaviour
- [ ] #8 Test: dispatch any task end-to-end via /ai-sdlc execute, verify attestation lands without explicit sign-attestation invocation
- [ ] #9 AI_SDLC_SKIP_ATTESTATION_SIGN=1 env-var escape for cases where signing must be deferred (e.g. operator hand-resigning later)
<!-- AC:END -->
