...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `.github/workflows/ai-sdlc-review.yml` `on.pull_request` adds `paths-ignore` covering `spec/rfcs/**`, `docs/**`, `backlog/tasks/**`, `backlog/completed/**`, and root-level `*.md`
- [ ] #2 Verify behavior on a test PR: docs-only commit → no `ai-sdlc-review.yml` run; mixed (docs + code) commit → review runs normally
- [ ] #3 Verify `verify-attestation.yml` is NOT modified — it should still run on docs-only PRs to keep the `ai-sdlc/attestation` status check current (cheap, no API tokens)
- [ ] #4 If a docs-only PR somehow needs a review (rare — e.g., contributor PR that should still get scrutiny), document the manual override: `gh workflow run ai-sdlc-review.yml --ref <branch>`
- [ ] #5 CLAUDE.md updated under "## Review attestations" → "### CI behavior" noting that docs-only PRs skip the reviewer fan-out
<!-- AC:END -->
