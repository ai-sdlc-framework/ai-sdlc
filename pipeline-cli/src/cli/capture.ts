 ...
 * and querying capture records. Records are written to
 * `$ARTIFACTS_DIR/_captures/<id>.jsonl` — immutable JSONL files, one per
 * capture.
 *
 * Subcommands:
 *   file                    — record a new capture (the primary path)
 *   list                    — list captures with optional filters
 *   redact <id>             — scrub finding text (preserve audit trail)
 *   against-current-pr      — file a capture auto-detecting the current PR
 *   triage <id>             — apply a triage decision to a pending capture
...
import { writeCapture, applyTriageUpdate, redactCapture } from '../capture/capture-writer.js';
...
export function buildCaptureCli(): Argv {
  return yargs(hideBin(process.argv))
    ...
    .command(
      'file <finding>',
      'Record a new capture (primary operator path).',
      (y) =>
        y
          .positional('finding', {
            type: 'string',
      ...
      async (argv) => {
        ...
        if (argv.json) {
          ...
          const record = writeCapture({
            ...
            triage: (parsed.triage as CaptureTriageValue) ?? 'tbd',
            sourceType: 'ai-agent',
        ...
        let operator = argv.operator as string | undefined;
        if (!operator) {
          try {
            const { spawnSync } = await import('node:child_process');
            ...
            operator = r.stdout?.trim() || process.env.USER || 'unknown';
          } catch {
        ...
        const record = writeCapture({
          ...
          triage: (argv.triage as CaptureTriageValue) ?? 'tbd',
          sourceType: 'operator',
          operator,
    ...
    .command(
      ...
      async (argv) => {
        ...
        let redactedBy = argv.by as string | undefined;
        if (!redactedBy) {
          try {
            const { spawnSync } = await import('node:child_process');
            const r = spawnSync('git', ['config', 'user.email'], {
            ...
            redactedBy = r.stdout?.trim() || process.env.USER || 'unknown';
          } catch {
            redactedBy = process.env.USER || 'unknown';
          }
        }
    ...
    .command(
      'against-current-pr',
      'File a capture auto-detecting the current PR from the active git branch (RFC-0024 §OQ-12).',
      (y) =>
        y
          .option('finding', {
            alias: 'f',
    ...
    .command(
      'triage <id>',
      'Apply a triage decision to a pending capture (triage=tbd → terminal value).',
      (y) =>
        y
          .positional('id', {
            type: 'string',
      ...
      async (argv) => {
        ...
        let resolvedBy = argv.by as string | undefined;
        if (!resolvedBy) {
          try {
            const { spawnSync } = await import('node:child_process');
            const r = spawnSync('git', ['config', 'user.email'], {
            ...
            resolvedBy = r.stdout?.trim() || process.env.USER || 'unknown';
          } catch {
    ...
    .command(
      ...
      async (argv) => {
        ...
        // Read comment list from stdin (gh pr view --json reviews ... | cli-capture parse-pr-comments).
        // Use synchronous fd read so the command stays synchronous (no async yargs handler needed).
        let input = '';
        try {
