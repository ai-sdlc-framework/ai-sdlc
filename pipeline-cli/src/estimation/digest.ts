 ...
 *  - `calibrationState` — 3-state Q6 token: `uncalibrated` / `warming` /
 *                         `calibrated` (per §7.3; shared with Phase 5
 *                         formatter).
 *
 * ## Stage-A-coverage
 *
 ...
 *  - `calibrated`   — n ≥ 5
 */
export function calibrationState(n: number): CalibrationState {
