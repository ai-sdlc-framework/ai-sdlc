 ...
 *  - Q6 calibration state token formatter (DigestCalibrationState).
 */
...
describe('digestCalibrationState', () => {
  it('returns uncalibrated for n=0', () => expect(digestCalibrationState(0)).toBe('uncalibrated'));
  it('returns warming for 1 <= n < 5', () => {
    expect(digestCalibrationState(1)).toBe('warming');
    expect(digestCalibrationState(4)).toBe('warming');
  });
  it('returns calibrated for n >= 5', () => {
    expect(digestCalibrationState(5)).toBe('calibrated');
    expect(digestCalibrationState(100)).toBe('calibrated');
  });
});

// ── formatCalibrationStateToken ───────────────────────────────────────────

describe('digest: formatCalibrationStateToken', () => {
...
describe('generateDigest — empty', () => {
  it('returns zero counts when no data exists', () => {
    ...
    expect(digest.classes.every((c) => c.digestCalibrationState === 'uncalibrated')).toBe(true);
  });
});
...
describe('generateDigest — calibration stats', () => {
  it('computes meanBucketMiss and oneBucketMissRate', () => {
    ...
    expect(choreRow.digestCalibrationState).toBe('calibrated');
