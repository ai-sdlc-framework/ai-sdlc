/**
 * RFC-0025 §5 failure-mode classifier (AISDLC-270).
 *
 ...
 *
 * Default classification is `ambiguous` (OQ-1 recommendation: preserves
 * operator agency while honest about uncertainty).
 *
 ...
 *
 * The classifier is intentionally heuristic-first — it can only see the
 * failure signal, not the full pipeline context. Callers with richer
 * context should provide `ctx.subclassHint` and/or `ctx.severityAxes`.
 *
 * Per OQ-1 recommendation, the default classification is `ambiguous`
