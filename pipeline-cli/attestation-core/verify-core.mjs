 ...
 * (`verifyAttestation`, `sha256Hex`, `computeContentHashV3/4/5`,
 * `isAttestationEnvelopePath`, `isIgnoredForContentHash`,
 * `validateTrustedReviewers`, `ACCEPTED_SCHEMA_VERSIONS`) are received via
 * `bindRuntime()`, called once by each driver after it resolves the runtime
 * module in whatever way is appropriate for its context. Every exported
 * function below is otherwise IDENTICAL to the pre-AISDLC-566 monorepo-only
 * verifier — same Merkle primitives, same head-binding relaxations, same
 * content-hash matching.
 *
 * AISDLC-84 (original doc, retained): rebase-stable matching. The verifier no longer matches envelopes
...
/**
 * AISDLC-566 — runtime bindings supplied by the driver.
 *
 * These names used to be a static `import { ... } from
 * '../orchestrator/dist/runtime/attestations.js'` at module scope — a path
 * that only resolves inside this monorepo checkout. Every function below
 * that references one of these names still does so via a plain identifier
 * (closure over this module-scope `let`), so the rest of the file is
 * byte-for-byte unchanged; only the *source* of these bindings moved from
 * "static monorepo-relative import" to "driver calls `bindRuntime()` once
 * before invoking anything else in this module".
 */
let ACCEPTED_SCHEMA_VERSIONS;
let verifyAttestation;
let sha256Hex;
let computeContentHashV3;
let computeContentHashV4;
let computeContentHashV5;
 ...
 * called by the driver before any other export in this module is invoked.
 *
 * @param {object} mod — the resolved `@ai-sdlc/orchestrator/runtime`
 *   (or `orchestrator/dist/runtime/attestations.js`) module namespace.
 */
export function bindRuntime(mod) {
  ({
...
// AISDLC-566: no CLI driver here — this module is a library imported by
// `scripts/verify-attestation.mjs` (repo-root) and
// `ai-sdlc-plugin/scripts/verify-attestation.mjs` (consumer-runnable), each
// of which owns its own `invokedDirectly` / argv / env-var handling.
