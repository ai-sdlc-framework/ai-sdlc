...
4. **`scripts/check-backlog-drift-on-push.sh`** (AISDLC-486) — catches error-severity Backlog Drift BEFORE push so the required CI "Backlog Drift" gate is satisfied locally. Two checks: (a) inbound-reference scan — for every file renamed/moved/deleted in the push range, greps `backlog/` for references to the OLD path; any match means a CI failure will occur; (b) task-level drift scan — runs `npx backlog-drift check --since <merge-base>` on tasks touched in this push and surfaces error-severity issues. Runs AFTER DoR (both block the push) and BEFORE the fixup orchestrator. **Motivating incident:** PR #789 renamed `commands/review.md` but did not update the backlog task AISDLC-71 that referenced the old path — this gate would have caught it locally. Skip: `AI_SDLC_SKIP_BACKLOG_DRIFT_PUSH_GATE=1`. Hermetic tests: `scripts/check-backlog-drift-on-push.test.mjs` (run via `pnpm test:backlog-drift-push-gate`).
5. **`scripts/pre-push-fixups.sh`** (AISDLC-386) — orchestrates two mechanical fixup sub-hooks in dependency order (task-move → attestation-sign) in a single pass. (mcp-bundle-sync removed by AISDLC-385 — bundle now distributed via npm.) Each sub-hook is invoked with `AI_SDLC_INTERNAL_NO_EXIT_1=1` so it does its work but exits 0 instead of 1. After all sub-hooks complete, if any fixup ran, the orchestrator exits 1 ONCE with a consolidated "re-run git push" message. This collapses the worst-case 3-push chain into 2. Exit 0 silently when no fixups are needed.
...
`set -euo pipefail` aborts on first failure. `git push --no-verify` bypasses everything. All gates have hermetic tests at `scripts/<name>.test.mjs` wired via `pnpm test:drift-gate` / `test:task-move-gate` / `test:dor-gate` / `test:attestation-sign-gate` / `test:pre-push-fixups-gate` / `test:backlog-drift-push-gate`. (`test:mcp-bundle-sync-gate` removed by AISDLC-385.)
