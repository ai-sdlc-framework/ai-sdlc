...
**Attestation is required.** `/ai-sdlc execute` runs three reviewer subagents locally and writes a DSSE envelope to `.ai-sdlc/attestations/<sha>.v6.dsse.json` (v6, default since RFC-0042 Phase 3 / AISDLC-383.6). `verify-attestation.yml` posts `ai-sdlc/attestation: success/failure` (required status on `main` per AISDLC-193). Missing/invalid envelopes block merge. `ai-sdlc-review.yml`'s `Post Review Results` is the parallel review-tier required check (CI-side reviewers run when local attestation is missing as the cost-saver fallback).

**Default schema: v6 (RFC-0042 Phase 3).** New envelopes use `schemaVersion: 'v6'` — a Merkle-transcript-based attestation. The signer reads transcript leaves from `.ai-sdlc/transcript-leaves.jsonl`, builds an RFC-6962 Merkle tree, signs the root with the operator's key, and writes `.ai-sdlc/attestations/<head-sha>.v6.dsse.json`. The verifier (AISDLC-383.4) verifies the Merkle proof + root signature. The `check-attestation-sign.sh` hook's `AI_SDLC_SCHEMA_VERSION` env var controls the version (`v6` default; `v5` explicit for legacy path). Use `--schema-version v5` flag on `sign-attestation.mjs` for the legacy v5 path.
...
**AISDLC-380 sub-attestation gate is audit-only (RFC-0042 Phase 3).** The pre-push hook Step 4d that verified per-reviewer signed sub-attestations is downgraded: on v6 envelopes (default) the gate is skipped entirely; on v5 envelopes it runs but failures are warnings only (exit 0, push not blocked). The gate code and sub-attestation scripts will be deleted in AISDLC-383.7. AISDLC-380.2 (architectural follow-up to close nonce/Read-tool bypasses) is marked Superseded — RFC-0042's Merkle-transcript model eliminates the attack surface it was addressing.
