...
- **Docs-only PRs also skip attestation verification (AISDLC-132).** `verify-attestation.yml`'s `pull_request` trigger carries the same `paths-ignore` block as `ai-sdlc-review.yml` — when ALL changed files match, neither workflow fires, so the `ai-sdlc/attestation` status check simply doesn't post for that PR. This is intentional: with the reviewer fan-out skipped (AISDLC-131), the CI-side attestor (which gates on 3 reviewer approvals) never signs, so a posted-but-invalid status would block merge with no recovery path. The `merge_group` trigger remains unfiltered so AISDLC-113's defense-in-depth verification against the queue head still runs once the PR enters the queue. Branch-protection note: `ai-sdlc/attestation` should be configured as a non-required check, OR if required, the branch-protection rule must treat "not posted" as N/A rather than failing — GitHub's default for required checks under merge-queue varies by repo configuration.
- **Manual override for docs-only PRs that need review** (rare — e.g. external-contributor scrutiny on a docs change, or a docs PR that touches policy you want a second pair of eyes on): `gh workflow run ai-sdlc-review.yml --ref <branch>`. This dispatches the workflow against the named branch even though the `paths-ignore` filter would otherwise skip it. To force attestation verification on a docs-only PR, similarly: `gh workflow run verify-attestation.yml --ref <branch>`.
