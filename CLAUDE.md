...
- **Docs-only PRs skip the reviewer fan-out (AISDLC-131).** `ai-sdlc-review.yml`'s `pull_request` trigger carries a `paths-ignore` block covering `spec/rfcs/**`, `docs/**`, `backlog/tasks/**`, `backlog/completed/**`, and root-level `*.md`. When ALL changed files in a PR match those patterns, GitHub doesn't fire the workflow — saving ~3 reviewer-agent invocations per push (verified: 8 review runs in ~1 hour on a single RFC-drafting session = ~24 wasted API calls). Mixed PRs (docs + code) still trigger normally — `paths-ignore` is "skip if all match", not "skip if any match".
- **Docs-only PRs also skip attestation verification (AISDLC-132).** `verify-attestation.yml`'s `pull_request` trigger carries the same `paths-ignore` block as `ai-sdlc-review.yml` — when ALL changed files match, neither workflow fires, so the `ai-sdlc/attestation` status check simply doesn't post for that PR. This is intentional: with the reviewer fan-out skipped (AISDLC-131), the CI-side attestor (which gates on 3 reviewer approvals) never signs, so a posted-but-invalid status would block merge with no recovery path. The `merge_group` trigger remains unfiltered so AISDLC-113's defense-in-depth verification against the queue head still runs once the PR enters the queue. Branch-protection note: `ai-sdlc/attestation` is NOT in the required-status-checks list on `main` — the workflow not posting on docs-only PRs is therefore not a merge blocker.
- **Docs-only PRs get `Post Review Results: success` posted by a fallback workflow (AISDLC-136).** Branch protection on `main` requires `Post Review Results` as a status check by name. Without intervention, AISDLC-131's `paths-ignore` would deadlock docs-only PRs (workflow skipped → required check never posts → un-mergeable; observed on PR #166). The orthogonal workflow `.github/workflows/ai-sdlc-review-docs-only.yml` runs on every `pull_request`, detects whether all changed files match the docs-only paths-ignore predicate (mirrored exactly), and if so posts `Post Review Results: success` directly via the GitHub status API. For mixed PRs (docs + code) the fallback exits cleanly — `ai-sdlc-review.yml`'s normal analyze + report path handles them. The fallback's concurrency group is shared with `ai-sdlc-review.yml` (`review-${{ github.event.pull_request.number }}`) but with `cancel-in-progress: false` so it never races the report job's status post on mixed PRs.
- **Manual override for docs-only PRs that need a real review** (rare — external-contributor scrutiny on a docs change, or a docs PR that touches policy you want a second pair of eyes on): there is no `workflow_dispatch` trigger on `ai-sdlc-review.yml` today (a prior version of this doc claimed there was — that was aspirational). To force a real review, push a tiny non-docs change (a single-character edit to any code file) so `paths-ignore` no longer matches all files. Then revert the throwaway change in a follow-up commit. The same applies to `verify-attestation.yml`. Adding a real `workflow_dispatch` path is tracked under follow-up work.
