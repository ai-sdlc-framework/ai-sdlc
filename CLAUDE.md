...
- **`AI_SDLC_TUI`** (RFC-0023): gates the operator TUI (`cli-tui` / `pnpm tui`). Off by default. Canonical opt-in value: `experimental` (other truthy values `1`/`true`/`yes`/`on` accepted, case-insensitive). When unset the TUI exits with a "not enabled" message + pointer to the promotion runbook. Phases 1-6 ship the Overview Mode, mode router, blockers/PRs/critical-path/analytics panes, and `_operator/{interactions,decisions,pr-decisions}.jsonl` writers behind the same flag. Phase 7 ships the `_tui/events.jsonl` self-observability writer (`TuiStarted`/`TuiCrashed`), the corpus aggregator (`cli-tui-corpus aggregate`), and the hybrid promotion runbook at [`docs/operations/operator-tui-promotion.md`](docs/operations/operator-tui-promotion.md) — operators dispatch the default-on flip from there once the corpus (≥7 sessions across ≥7 calendar days, ≥2 distinct panes opened, zero `TuiCrashed`) or spot-check evidence supports it. The companion `AI_SDLC_TUI_TELEMETRY=off` (RFC §10 OQ-8) is the kill switch for `_tui/events.jsonl` + `_operator/*.jsonl` local writes; defaults ON, opt-OUT only. See [`spec/rfcs/RFC-0023-operator-tui-pipeline-monitoring.md`](spec/rfcs/RFC-0023-operator-tui-pipeline-monitoring.md).
