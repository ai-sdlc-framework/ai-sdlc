...
4. **`scripts/check-attestation-sign.sh`** — auto-signs DSSE attestation when `<worktree>/.active-task` exists, `<worktree>/.ai-sdlc/verdicts/<task-id-lower>.json` exists, and no envelope at HEAD. Commits the envelope as a separate `chore: auto-sign attestation for <task-id>` and exits 1 with "re-run git push". Idempotent on the second push (envelope-at-HEAD or HEAD-is-auto-sign-chore predicate). When no verdict file exists (docs-only PRs, chore commits, ad-hoc pushes), exits 0 as a no-op — docs-only PRs are handled entirely by CI (AISDLC-214 short-circuits `verify-attestation.yml` with a direct `ai-sdlc/attestation: success` status per RFC-0042 Phase 3). Skip: `AI_SDLC_SKIP_ATTESTATION_SIGN=1`.
