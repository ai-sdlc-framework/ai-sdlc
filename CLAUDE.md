...
**Attestation is required.** `/ai-sdlc execute` runs three reviewer subagents locally and writes a DSSE envelope to `.ai-sdlc/attestations/<sha>.dsse.json`. `verify-attestation.yml` posts `ai-sdlc/attestation: success/failure` (required status on `main` per AISDLC-193). Missing/invalid envelopes block merge. `ai-sdlc-review.yml`'s `Post Review Results` is the parallel review-tier required check (CI-side reviewers run when local attestation is missing as the cost-saver fallback). `contentHashV3` (`sha256("<baseBlobSha> -> <headBlobSha>")` per changed file) is partially rebase-stable — invalidates when sibling PRs touch the same files. AISDLC-193 stage 2 ships `contentHashV4` (per-file `{path, headBlobSha}` map, base-independent) to fix the queue-rebase invalidation. Docs-only PRs (`spec/rfcs/**`, `docs/**`, `backlog/{tasks,completed}/**`, root `*.md`) skip both `ai-sdlc-review.yml` and `verify-attestation.yml` via `paths-ignore`; `ai-sdlc-review-docs-only.yml` posts `Post Review Results` directly so merge isn't deadlocked.
