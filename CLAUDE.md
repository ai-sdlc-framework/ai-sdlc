...
**Merge-queue rebase re-sign (AISDLC-397):** When the merge-queue's update-branch step rebases a PR onto fresh main, all commit SHAs change. The existing attestation envelope (named after the pre-rebase HEAD SHA) becomes stale — `verify-attestation.yml` posts failure because no envelope exists for the new HEAD. `.github/workflows/resign-attestation-on-rebase.yml` closes this loop automatically: it triggers on `pull_request.synchronize`, detects update-branch rebases (heuristic: pre-rebase SHA had an envelope, post-rebase SHA does not), and re-signs the envelope using a CI-side ed25519 key (`AI_SDLC_CI_SIGNING_KEY` repo secret). The chore commit is authored as `github-actions[bot]` and pushed with `--force-with-lease`. Short-circuits if no `.active-task` sentinel exists (not an AI-SDLC task PR), no verdict file is present (PR was never attested), or the HEAD commit is already a re-sign chore (loop guard). **To provision the CI signing key:** run `node ai-sdlc-plugin/scripts/init-signing-key.mjs --pem-only | base64 | gh secret set AI_SDLC_CI_SIGNING_KEY` and add the printed public key to `.ai-sdlc/trusted-reviewers.yaml` under the `ci-resign-bot` entry. See the workflow header for the full runbook. Motivating incident: PR #626 (AISDLC-373) cycled 3 times in the overnight 2026-05-23 autonomous session before being marked STUCK.
