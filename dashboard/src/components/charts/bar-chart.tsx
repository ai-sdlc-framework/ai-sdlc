...
export function BarChart({ data, width = 400, height = 200, barColor = '#3b82f6' }: BarChartProps) {
    ...
    <svg width={width} height={height} role="img" aria-label="Bar chart">
      <g transform={`translate(${padding.left},${padding.top})`}>
        {data.map((d, i) => {
          ...
          return (
            <g key={d.label}>
              <rect x={x} y={y} width={barWidth} height={barH} fill={d.color ?? barColor} rx={2} />
              <text
                x={x + barWidth / 2}
                y={chartH + 16}
                textAnchor="middle"
              ...
              <text x={x + barWidth / 2} y={y - 4} textAnchor="middle" fontSize={10} fill="#334155">
