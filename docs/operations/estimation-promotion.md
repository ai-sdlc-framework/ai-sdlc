...
`AI_SDLC_ESTIMATION_CALIBRATION=experimental` to default-on.
...
| 3 | **Stage-A-coverage >70%** | `cli-estimate digest` | `stageACoverageRate > 0.70` per class |
...
# Per-class detail
node pipeline-cli/bin/cli-estimate.mjs show feature --format table
node pipeline-cli/bin/cli-estimate.mjs show bug --format table
node pipeline-cli/bin/cli-estimate.mjs show chore --format table
...
1. Run `cli-estimate show <class> --format table` for each class and
   document the output.
...
2. Verify the weekly digest produces a non-empty result:
   `node pipeline-cli/bin/cli-estimate.mjs digest --format table`
3. Monitor for `EstimateBiasOverCorrected` events in `events.jsonl`
   (indicates the bias multiplier needs re-tuning per RFC §7.4).
...
| `cli-estimate show <class>` | Per-class calibration stats + Stage A/B accuracy |
| `cli-estimate digest` | Weekly digest across all classes (Phase 6) |
