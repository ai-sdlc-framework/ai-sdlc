...
**Proof of Execution (PoE) is a cryptographic primitive that closes this gap.** It produces tamper-evident, append-only, signed evidence that a real review process ran against the specific diff being merged by a process with repo access — evidence that survives rebases, costs effectively zero per PR, requires no third-party transparency service, and is verifiable retrospectively against any historical commit. **PoE does not, by itself, prove the reviewer's identity or that the reviewer was independent of the coordinator that made the ship decision** — see §2.2 for the current scope boundary and AISDLC-568 for the tracked follow-up.
...
| **Coordinator self-review masquerading as independent review** (AISDLC-568) | Today's transcript-capture contract is a Bash sequence the reviewer subagent's own process executes against its own conversation. Nothing structurally distinguishes a transcript written by a genuinely separate reviewer subagent from one written by the *same coordinator process* that also drove the developer subagent and makes the ship decision, as long as that coordinator is willing to pay the same LLM-token cost. PoE proves *a real review ran against this exact code state by a process with repo access* — it does NOT by itself prove reviewer identity or independence from the coordinator. Structurally anchoring independence (binding the leaf to the reviewer subagent's real, harness-captured execution transcript, and/or a distinct lower-trust verdict class for coordinator/self-authored attestations) is tracked as a follow-up (AISDLC-568) and requires an operator design walkthrough before implementation. |

The threat model is *insider/agent forgery within the operator's own pipeline*. PoE significantly raises the cost of the *unexecuted-review* forgery class without claiming to be a universal solution, and — as of this writing — does not yet structurally distinguish an independent reviewer from a self-reviewing coordinator (see the row above).
