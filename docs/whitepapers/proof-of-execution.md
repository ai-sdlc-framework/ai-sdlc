...
| **Adversarial coordinator forging the independence signal itself** (AISDLC-568, partial mitigation shipped) | Every v6 leaf now carries a `verdictClass: 'independent' \| 'self-authored'` field (DEC-0012 opt-b), derived from whether a real `SubagentStart` harness hook fired around the time the transcript was written — a signal a coordinator cannot trigger by writing files or text alone, only by actually dispatching an `Agent`/`Task` subagent. This correctly downgrades the common case (a coordinator running the reviewer's Bash steps itself, in-conversation, without spawning a subagent) to `self-authored`. It does NOT stop a *deliberately adversarial* coordinator with full repo Bash/Write access from also fabricating the marker file the hook writes — that requires binding the leaf to the reviewer subagent's own harness-captured execution transcript (DEC-0012 opt-a), which remains a harness-coupled follow-up, not yet implemented. |
