...
**Proof of Execution (PoE) is a cryptographic primitive that closes this gap.** It produces tamper-evident, append-only, signed evidence that a real review process ran against the specific diff being merged by a process with repo access — evidence that survives rebases, costs effectively zero per PR, requires no third-party transparency service, and is verifiable retrospectively against any historical commit. **PoE does not, by itself, prove the reviewer's identity, and its independence signal (`verdictClass`, AISDLC-568) is a same-process/session heuristic rather than a cryptographic guarantee** — see §2.2 for the current scope boundary.
...
|---|---|---|
| **Developer-agent forgery (unexecuted review)** | A developer LLM-agent fabricates a verdict file claiming reviewers approved when no reviewer process ran at all | Each reviewer transcript hash is committed *before* it can be signed; absence of the transcript = absence of evidence = no valid envelope |
...
| **Adversarial coordinator forging the independence signal itself** (AISDLC-568, partial mitigation shipped; role-bound since AISDLC-572) | Every v6 leaf now carries a `verdictClass: 'independent' \| 'self-authored'` field (DEC-0012 opt-b), derived from whether a real `SubagentStart` harness hook fired, **with a reviewer role** (`code-reviewer`, `test-reviewer`, `security-reviewer`, or their `-codex` variants), around the time the transcript was written — a signal a coordinator cannot trigger by writing files or text alone, only by actually dispatching an `Agent`/`Task` reviewer subagent. AISDLC-572 closed a gap where the signal was credited on timing alone: any subagent (including a `developer` or `rebase-resolver`) dispatched in the window made a leaf `independent`, so a coordinator could dispatch an unrelated subagent and then hand-author a reviewer transcript within the window and still get credited. Binding the marker to the actual `agent_type` on the wire (and narrowing the lookback window from 2h to 30 minutes) correctly downgrades that case, plus the original common case (a coordinator running the reviewer's Bash steps itself, in-conversation, without spawning a subagent), to `self-authored`. It does NOT stop a *deliberately adversarial* coordinator with full repo Bash/Write access from also fabricating the marker file the hook writes — INCLUDING forging a reviewer `agent_type` string — see the next row. |
| **Adversarial coordinator fabricating the `SubagentStart` marker file directly** (AISDLC-570, DEC-0012 opt-a, DEC-0013 → opt1, shipped) | Every v6 leaf may additionally carry an optional `harnessTranscriptHash` field: the SHA-256 of the reviewer subagent's own harness-captured execution transcript (Claude Code auto-writes every subagent invocation to `~/.claude/projects/<slug>/<session-id>/subagents/agent-<id>.jsonl` — a file the coordinator did not author and cannot cheaply fabricate without paying for a real `Agent`/`Task` call). Set only when the resolved transcript's reported role is a reviewer AND it contains a diff-binding nonce embedded in the reviewer's dispatch prompt — any failure fails closed to `null`. **Trust-model caveat (DEC-0013):** this is verified ONCE, at sign time, on the operator's own machine — a downstream CI verifier cannot independently re-derive it (no `~/.claude/projects/` on a CI runner), though tampering with an already-signed value is still caught by the Merkle root signature. It also does not prove the subagent's judgment was free of coordinator-engineered prompt bias — a coordinator willing to pay the LLM-token cost can still spawn a real, but biased, reviewer subagent (see the "Compromised reviewer prompts" row above). **Wiring status: ACTIVE (AISDLC-573).** Both canonical reviewer-dispatch paths (`/ai-sdlc execute` and `/ai-sdlc orchestrator-tick`) now generate the nonce and embed it in each reviewer's prompt before dispatch, so this field populates for real reviewer runs. The trust-model caveats above are unaffected — activation closes the wiring gap, not the sign-time-only / prompt-bias limitations. |

The threat model is *insider/agent forgery within the operator's own pipeline*. PoE significantly raises the cost of the *unexecuted-review* forgery class without claiming to be a universal solution. As of this writing it structurally distinguishes an independent reviewer subagent from a lazily self-reviewing coordinator (the common case) via `verdictClass`, and additionally raises the forgery floor against a deliberately adversarial coordinator via `harnessTranscriptHash` — though the latter is a sign-time-only signal, not a CI-re-derivable one, and neither closes the prompt-bias gap (see the table rows above).
