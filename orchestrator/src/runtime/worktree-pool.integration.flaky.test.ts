 ...
 *
 * FLAKY: The 3-worktree parallel-allocate test exhibits a git worktree
 * write-then-read race on CI under load (observed 2+x, AISDLC-368). This file
 * is excluded from the default vitest run via the "**\/*.flaky.test.ts" exclude
 * pattern in vitest.config.ts and is exercised by the nightly
 * .github/workflows/flaky-tests.yml workflow instead.
 *
