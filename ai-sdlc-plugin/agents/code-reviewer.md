...
if [ -n "$RESOLVE_SCRIPT" ]; then
  TASK_ID="$(bash "$RESOLVE_SCRIPT" code-reviewer)" || {
  ...
  TASK_ID="UNKNOWN-code-reviewer-$(date -u +%Y%m%dT%H%M%S)-$RANDOM"
...
printf '%s' "$TASK_ID" > .ai-sdlc/transcripts/.last-task-id.code-reviewer
...
```bash
TASK_ID="$(cat .ai-sdlc/transcripts/.last-task-id.code-reviewer 2>/dev/null)"
[ -z "$TASK_ID" ] && TASK_ID="UNKNOWN-code-reviewer-$(date -u +%Y%m%dT%H%M%S)-$RANDOM"
