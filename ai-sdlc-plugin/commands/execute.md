...
Items 1-4 and 6 below are **governance-configurable per repo** (`.ai-sdlc/agent-role.yaml` `spec.governance` — RFC-0048 / AISDLC-601). Before relying on the static text, run this and treat its output as authoritative for the rest of the session:

```bash
node "${CLAUDE_PLUGIN_ROOT:-$(pwd)/ai-sdlc-plugin}/scripts/render-governance-hard-rules.mjs"
```

This prints the SAME resolved-policy bullets the SessionStart/SubagentStart governance banners already showed you at session start (AISDLC-601's `renderSubagentHardRules()` — one source of truth, no duplicated rule text). With no `governance:` section in `agent-role.yaml` the output is byte-identical in substance to the strict text below, so nothing changes for the common case. Items 5 and 7 are **fixed integrity floors, never governance-configurable** (RFC-0048 OQ-3) — they apply regardless of what the script prints.

1. **Never merge any PR — unless the script's output says otherwise (`allowMerge: onGreenClean`).** Do not run `gh pr merge` directly under any circumstance. Even when the resolved policy permits merge-on-green, the ONLY sanctioned merge path is `node pipeline-cli/bin/cli-merge-if-eligible.mjs` (AISDLC-603), which enforces the real green+CLEAN+trusted-`sourceKind` gate — never reimplement that check yourself. The PreToolUse hook (`enforce-blocked-actions.js`, AISDLC-602) blocks a raw `gh pr merge` invocation regardless of policy. Arming auto-merge (`gh pr merge --auto`) is NOT merging and stays allowed under every policy, including strict.
2. **Never force-push — unless the script's output says otherwise (`allowForcePush: true`).** No `git push --force` / `-f` under strict. If push fails (non-fast-forward) under strict, abort with a clear message and ask the operator. When the policy allows force-push, still use `--force-with-lease`, never bare `--force`.
3. **Never close PRs or issues — unless the script's output says otherwise (`allowClosePrIssue: true`).** No `gh pr close`, `gh issue close` under strict.
4. **Never delete branches — unless the script's output says otherwise (`allowBranchDelete: true`).** No `git branch -D` / `-d` under strict.
5. **Never edit `.ai-sdlc/**`.** Configuration is out of scope for task work — the PreToolUse hook also blocks this, but you must not even try. `.github/workflows/**` is only refused when the project's `.ai-sdlc/agent-role.yaml` lists it under `blockedPaths` — not blocked by default. **Not governance-configurable** — this is a permanently-fixed integrity floor (RFC-0048 OQ-3), never representable via `spec.governance`.
6. **Never run `git reset --hard` ad-hoc — unless the script's output says otherwise (`allowResetHard: true`).** Under strict, the sanctioned path is `scripts/check-orchestrator-state.sh`, which runs `git reset --hard origin/main` ONLY when the parent working tree is verifiably clean. Outside that script, `git reset --hard` is forbidden unless the operator explicitly authorizes it in the current session (or the resolved policy opts in). `git checkout -- .` and `git restore .` are always forbidden regardless of policy. (AISDLC-450)
7. **Never write GitHub Actions CI-skip magic tokens into commit messages (AISDLC-88).** GitHub Actions parses five literal substrings — `[skip ci]`, `[ci skip]`, `[no ci]`, `[skip actions]`, `[actions skip]` — case-insensitively, and SUPPRESSES every workflow on commits that carry any of them. That silently disables verify-attestation and ai-sdlc-review in one stroke. If you genuinely need to mention these tokens in a commit body, use the **paren-quoted form**: `(skip ci marker)` instead of `[skip ci]`. Backtick-wrapping (`` `[skip ci]` ``) does NOT defeat the parser — the literal bracketed substring is still present. The `.husky/pre-push` `check-skip-ci-marker.sh` gate (AISDLC-88) blocks pushes that violate this. The legacy AISDLC-87 CI-side attestor's `chore(ci): sign review attestation [skip ci]` commits (authored by `ai-sdlc-ci-attestor[bot]`; legacy `github-actions[bot]` retained as a fallback) are still exempted so historical commits replayed via auto-rebase don't strand pushes — but the attestor itself was removed in AISDLC-140 sub-4 (attestation is now audit-only) and AISDLC-152 (this task), so no NEW chore commits should be produced. Step 10 below additionally sanitises any leaked tokens out of the chore-commit body before staging, as defense-in-depth. **Not governance-configurable** — this is a permanently-fixed integrity floor (RFC-0048 OQ-3), never representable via `spec.governance`.
...
- **Never edits `.ai-sdlc/**`.** PreToolUse hook blocks anyway, but the developer prompt makes this explicit. `.github/workflows/**` is only refused when the project's `.ai-sdlc/agent-role.yaml` lists it under `blockedPaths`.
