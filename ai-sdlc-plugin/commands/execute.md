...
5. **Never edit `.ai-sdlc/**`.** Configuration is out of scope for task work — the PreToolUse hook also blocks this, but you must not even try. `.github/workflows/**` is only refused when the project's `.ai-sdlc/agent-role.yaml` lists it under `blockedPaths` — not blocked by default.
